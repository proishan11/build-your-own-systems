...
How teams decide when a measured performance change is unacceptable.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How teams decide when a measured performance change is unacceptable. |
...
Think of regression thresholds as a measured feedback loop from workload to symptom to profile to change to regression guard.
...
For regression thresholds, the invariant is:

> The system must preserve the explicit state contract for how teams decide when a measured performance change is unacceptable, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on regression thresholds. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| performance-engineering and SRE ladders | Use this chapter before opening project-specific placeholders that rely on regression thresholds. |
...
You are ready to implement an exercise using regression thresholds when you can:
...
1. Which state is made explicit by regression thresholds that would otherwise be hidden?
