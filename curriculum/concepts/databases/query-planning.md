...
How a database chooses a physical strategy for a declarative query.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How a database chooses a physical strategy for a declarative query. |
...
Think of query planning as a layered engine that transforms declarative intent into storage, execution, and transactional behavior.
...
For query planning, the invariant is:

> The system must preserve the explicit state contract for how a database chooses a physical strategy for a declarative query, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on query planning. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| database-systems and query-optimizer ladders | Use this chapter before opening project-specific placeholders that rely on query planning. |
...
You are ready to implement an exercise using query planning when you can:
...
1. Which state is made explicit by query planning that would otherwise be hidden?
