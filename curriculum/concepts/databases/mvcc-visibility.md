...
How transactions decide which row versions they are allowed to see.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How transactions decide which row versions they are allowed to see. |
...
Think of mVCC visibility as a layered engine that transforms declarative intent into storage, execution, and transactional behavior.
...
For mVCC visibility, the invariant is:

> The system must preserve the explicit state contract for how transactions decide which row versions they are allowed to see, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on mVCC visibility. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| database-systems and query-optimizer ladders | Use this chapter before opening project-specific placeholders that rely on mVCC visibility. |
...
You are ready to implement an exercise using mVCC visibility when you can:
...
1. Which state is made explicit by mVCC visibility that would otherwise be hidden?
