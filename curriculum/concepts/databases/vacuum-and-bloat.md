...
How databases reclaim obsolete versions and keep tables healthy.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How databases reclaim obsolete versions and keep tables healthy. |
...
Think of vacuum and bloat as a layered engine that transforms declarative intent into storage, execution, and transactional behavior.
...
For vacuum and bloat, the invariant is:

> The system must preserve the explicit state contract for how databases reclaim obsolete versions and keep tables healthy, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on vacuum and bloat. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| database-systems and query-optimizer ladders | Use this chapter before opening project-specific placeholders that rely on vacuum and bloat. |
...
You are ready to implement an exercise using vacuum and bloat when you can:
...
1. Which state is made explicit by vacuum and bloat that would otherwise be hidden?
