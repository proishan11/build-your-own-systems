...
How training state is persisted so work can resume or roll back.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How training state is persisted so work can resume or roll back. |
...
Think of checkpointing as a pipeline that moves tensors and metadata through training, checkpointing, serving, and monitoring.
...
For checkpointing, the invariant is:

> The system must preserve the explicit state contract for how training state is persisted so work can resume or roll back, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on checkpointing. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| ml-systems ladders | Use this chapter before opening project-specific placeholders that rely on checkpointing. |
...
You are ready to implement an exercise using checkpointing when you can:
...
1. Which state is made explicit by checkpointing that would otherwise be hidden?
