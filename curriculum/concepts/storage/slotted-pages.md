...
How database pages store variable-length records while keeping stable item references.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How database pages store variable-length records while keeping stable item references. |
...
Think of slotted pages as a set of on-disk promises guarded by explicit formats and recovery rules.
...
For slotted pages, the invariant is:

> The system must preserve the explicit state contract for how database pages store variable-length records while keeping stable item references, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on slotted pages. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| database/002-slotted-page | database/002-slotted-page exercises record placement, stable slots, free space, and deletion behavior. |
| database-systems and distributed durable-log ladders | Use this chapter to identify the invariant and write edge-case tests before implementation. |
| Staff review | Ask whether the implementation preserves the slotted pages contract under failure and scale. |
...
You are ready to implement an exercise using slotted pages when you can:
...
1. Which state is made explicit by slotted pages that would otherwise be hidden?
