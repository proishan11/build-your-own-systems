...
How a database manages a limited in-memory cache of disk pages.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How a database manages a limited in-memory cache of disk pages. |
...
Think of buffer pools as a set of on-disk promises guarded by explicit formats and recovery rules.
...
For buffer pools, the invariant is:

> The system must preserve the explicit state contract for how a database manages a limited in-memory cache of disk pages, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on buffer pools. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| database/003-buffer-pool | database/003-buffer-pool exercises pinning, eviction, dirty state, and page lifecycle. |
...
| Staff review | Ask whether the implementation preserves the buffer pools contract under failure and scale. |
...
You are ready to implement an exercise using buffer pools when you can:
...
1. Which state is made explicit by buffer pools that would otherwise be hidden?
