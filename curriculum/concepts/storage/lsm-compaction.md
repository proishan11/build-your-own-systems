...
How log-structured stores merge sorted runs without losing key-version semantics.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How log-structured stores merge sorted runs without losing key-version semantics. |
...
Think of lSM compaction as a set of on-disk promises guarded by explicit formats and recovery rules.
...
For lSM compaction, the invariant is:

> The system must preserve the explicit state contract for how log-structured stores merge sorted runs without losing key-version semantics, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on lSM compaction. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| database-systems and distributed durable-log ladders | Use this chapter before opening project-specific placeholders that rely on lSM compaction. |
...
You are ready to implement an exercise using lSM compaction when you can:
...
1. Which state is made explicit by lSM compaction that would otherwise be hidden?
