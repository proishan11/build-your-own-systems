...
How software turns buffered writes into a crash-surviving promise.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How software turns buffered writes into a crash-surviving promise. |
...
Think of durable writes and fsync as a set of on-disk promises guarded by explicit formats and recovery rules.
...
For durable writes and fsync, the invariant is:

> The system must preserve the explicit state contract for how software turns buffered writes into a crash-surviving promise, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on durable writes and fsync. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| database-systems and distributed durable-log ladders | Use this chapter before opening project-specific placeholders that rely on durable writes and fsync. |
...
You are ready to implement an exercise using durable writes and fsync when you can:
...
1. Which state is made explicit by durable writes and fsync that would otherwise be hidden?
