...
How requests are validated or mutated before they persist.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How requests are validated or mutated before they persist. |
...
Think of admission webhooks as declarative control loops over API objects plus lower-level Linux isolation primitives.
...
For admission webhooks, the invariant is:

> The system must preserve the explicit state contract for how requests are validated or mutated before they persist, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on admission webhooks. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| containers-kubernetes ladders | Use this chapter before opening project-specific placeholders that rely on admission webhooks. |
...
You are ready to implement an exercise using admission webhooks when you can:
...
1. Which state is made explicit by admission webhooks that would otherwise be hidden?
