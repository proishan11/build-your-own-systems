...
How container images package filesystems, manifests, and layers.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How container images package filesystems, manifests, and layers. |
...
Think of oCI image layout as declarative control loops over API objects plus lower-level Linux isolation primitives.
...
For oCI image layout, the invariant is:

> The system must preserve the explicit state contract for how container images package filesystems, manifests, and layers, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on oCI image layout. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| containers-kubernetes ladders | Use this chapter before opening project-specific placeholders that rely on oCI image layout. |
...
You are ready to implement an exercise using oCI image layout when you can:
...
1. Which state is made explicit by oCI image layout that would otherwise be hidden?
