...
How Kubernetes authorizes API verbs over resources and subjects.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How Kubernetes authorizes API verbs over resources and subjects. |
...
Think of rBAC as declarative control loops over API objects plus lower-level Linux isolation primitives.
...
For rBAC, the invariant is:

> The system must preserve the explicit state contract for how Kubernetes authorizes API verbs over resources and subjects, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on rBAC. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| containers-kubernetes ladders | Use this chapter before opening project-specific placeholders that rely on rBAC. |
...
You are ready to implement an exercise using rBAC when you can:
...
1. Which state is made explicit by rBAC that would otherwise be hidden?
