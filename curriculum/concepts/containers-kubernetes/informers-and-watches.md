...
How controllers keep local caches synchronized with API changes.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How controllers keep local caches synchronized with API changes. |
...
Think of informers and watches as declarative control loops over API objects plus lower-level Linux isolation primitives.
...
For informers and watches, the invariant is:

> The system must preserve the explicit state contract for how controllers keep local caches synchronized with API changes, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on informers and watches. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| containers-kubernetes ladders | Use this chapter before opening project-specific placeholders that rely on informers and watches. |
...
You are ready to implement an exercise using informers and watches when you can:
...
1. Which state is made explicit by informers and watches that would otherwise be hidden?
