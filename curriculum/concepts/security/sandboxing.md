...
How software confines untrusted code, files, network, and capabilities.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How software confines untrusted code, files, network, and capabilities. |
...
Think of sandboxing as assets, actors, trust boundaries, controls, and evidence arranged around abuse cases.
...
For sandboxing, the invariant is:

> The system must preserve the explicit state contract for how software confines untrusted code, files, network, and capabilities, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on sandboxing. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| security-engineering and agent-security ladders | Use this chapter before opening project-specific placeholders that rely on sandboxing. |
...
You are ready to implement an exercise using sandboxing when you can:
...
1. Which state is made explicit by sandboxing that would otherwise be hidden?
