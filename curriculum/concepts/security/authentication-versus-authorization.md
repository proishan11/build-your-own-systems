...
How systems separate proving identity from granting permission.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How systems separate proving identity from granting permission. |
...
Think of authentication versus authorization as assets, actors, trust boundaries, controls, and evidence arranged around abuse cases.
...
For authentication versus authorization, the invariant is:

> The system must preserve the explicit state contract for how systems separate proving identity from granting permission, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on authentication versus authorization. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| security-engineering and agent-security ladders | Use this chapter before opening project-specific placeholders that rely on authentication versus authorization. |
...
You are ready to implement an exercise using authentication versus authorization when you can:
...
1. Which state is made explicit by authentication versus authorization that would otherwise be hidden?
