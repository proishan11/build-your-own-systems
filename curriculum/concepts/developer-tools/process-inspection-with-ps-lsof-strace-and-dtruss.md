# Process Inspection with ps, lsof, strace, and dtruss
...
How local observability tools reveal processes, descriptors, syscalls, and resource ownership.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How local observability tools reveal processes, descriptors, syscalls, and resource ownership. |
...
Think of process inspection with ps, lsof, strace, and dtruss as a small contract between a human command, a process, and the operating system.
...
For process inspection with ps, lsof, strace, and dtruss, the invariant is:

> The system must preserve the explicit state contract for how local observability tools reveal processes, descriptors, syscalls, and resource ownership, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on process inspection with ps, lsof, strace, and dtruss. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| tooling-foundations and systems-programming ladders | Use this chapter before opening project-specific placeholders that rely on process inspection with ps, lsof, strace, and dtruss. |
...
You are ready to implement an exercise using process inspection with ps, lsof, strace, and dtruss when you can:
...
1. Which state is made explicit by process inspection with ps, lsof, strace, and dtruss that would otherwise be hidden?
