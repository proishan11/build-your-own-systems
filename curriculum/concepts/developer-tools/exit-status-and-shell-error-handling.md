...
How commands communicate success, failure, and partial failure to scripts and humans.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How commands communicate success, failure, and partial failure to scripts and humans. |
...
Think of exit status and shell error handling as a small contract between a human command, a process, and the operating system.
...
For exit status and shell error handling, the invariant is:

> The system must preserve the explicit state contract for how commands communicate success, failure, and partial failure to scripts and humans, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on exit status and shell error handling. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| tooling-foundations and systems-programming ladders | Use this chapter before opening project-specific placeholders that rely on exit status and shell error handling. |
...
You are ready to implement an exercise using exit status and shell error handling when you can:
...
1. Which state is made explicit by exit status and shell error handling that would otherwise be hidden?
