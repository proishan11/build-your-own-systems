...
How a gateway rewrites addresses while remembering flow identity.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How a gateway rewrites addresses while remembering flow identity. |
...
Think of nAT and connection tracking as distributed state machines exchanging bounded packets over unreliable links.
...
For nAT and connection tracking, the invariant is:

> The system must preserve the explicit state contract for how a gateway rewrites addresses while remembering flow identity, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on nAT and connection tracking. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| deep-networking and networking ladders | Use this chapter before opening project-specific placeholders that rely on nAT and connection tracking. |
...
You are ready to implement an exercise using nAT and connection tracking when you can:
...
1. Which state is made explicit by nAT and connection tracking that would otherwise be hidden?
