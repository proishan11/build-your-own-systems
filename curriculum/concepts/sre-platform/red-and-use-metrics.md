...
How services and resources get different but complementary telemetry views.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How services and resources get different but complementary telemetry views. |
...
Think of RED and USE metrics as a production feedback loop: define reliability, instrument behavior, control change, and learn from incidents.
...
For RED and USE metrics, the invariant is:

> The system must preserve the explicit state contract for how services and resources get different but complementary telemetry views, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on RED and USE metrics. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| sre-system-design and platform ladders | Use this chapter before opening project-specific placeholders that rely on RED and USE metrics. |
...
You are ready to implement an exercise using RED and USE metrics when you can:
...
1. Which state is made explicit by RED and USE metrics that would otherwise be hidden?
