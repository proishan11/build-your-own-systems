...
How replicas represent causality without a single global clock.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How replicas represent causality without a single global clock. |
...
Think of vector clocks as several state machines coordinating through messages, quorums, leases, or replicated logs.
...
For vector clocks, the invariant is:

> The system must preserve the explicit state contract for how replicas represent causality without a single global clock, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on vector clocks. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| distributed-systems ladders and database replication projects | Use this chapter before opening project-specific placeholders that rely on vector clocks. |
...
You are ready to implement an exercise using vector clocks when you can:
...
1. Which state is made explicit by vector clocks that would otherwise be hidden?
