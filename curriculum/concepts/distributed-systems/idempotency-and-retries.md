...
How clients safely repeat uncertain operations.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How clients safely repeat uncertain operations. |
...
Think of idempotency and retries as several state machines coordinating through messages, quorums, leases, or replicated logs.
...
For idempotency and retries, the invariant is:

> The system must preserve the explicit state contract for how clients safely repeat uncertain operations, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on idempotency and retries. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| distributed-systems ladders and database replication projects | Use this chapter before opening project-specific placeholders that rely on idempotency and retries. |
...
You are ready to implement an exercise using idempotency and retries when you can:
...
1. Which state is made explicit by idempotency and retries that would otherwise be hidden?
