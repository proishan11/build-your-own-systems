...
How overlapping majorities preserve knowledge across replica decisions.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How overlapping majorities preserve knowledge across replica decisions. |
...
Think of quorum intersection as several state machines coordinating through messages, quorums, leases, or replicated logs.
...
For quorum intersection, the invariant is:

> The system must preserve the explicit state contract for how overlapping majorities preserve knowledge across replica decisions, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on quorum intersection. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| distributed-systems ladders and database replication projects | Use this chapter before opening project-specific placeholders that rely on quorum intersection. |
...
You are ready to implement an exercise using quorum intersection when you can:
...
1. Which state is made explicit by quorum intersection that would otherwise be hidden?
