...
How a protocol states what kinds of crashes, delays, and corruptions it can tolerate.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How a protocol states what kinds of crashes, delays, and corruptions it can tolerate. |
...
Think of failure models as several state machines coordinating through messages, quorums, leases, or replicated logs.
...
For failure models, the invariant is:

> The system must preserve the explicit state contract for how a protocol states what kinds of crashes, delays, and corruptions it can tolerate, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on failure models. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| distributed-systems ladders and database replication projects | Use this chapter before opening project-specific placeholders that rely on failure models. |
...
You are ready to implement an exercise using failure models when you can:
...
1. Which state is made explicit by failure models that would otherwise be hidden?
