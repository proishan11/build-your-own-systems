...
How PostgreSQL controls dirty page flushing and crash recovery distance.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How PostgreSQL controls dirty page flushing and crash recovery distance. |
...
Think of checkpoints as a stateful database service whose data, WAL, locks, plans, and background workers must be observed together.
...
For checkpoints, the invariant is:

> The system must preserve the explicit state contract for how PostgreSQL controls dirty page flushing and crash recovery distance, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on checkpoints. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| postgres-administration ladders | Use this chapter before opening project-specific placeholders that rely on checkpoints. |
...
You are ready to implement an exercise using checkpoints when you can:
...
1. Which state is made explicit by checkpoints that would otherwise be hidden?
