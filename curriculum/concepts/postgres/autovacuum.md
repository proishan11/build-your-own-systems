...
How PostgreSQL automatically reclaims dead tuples and refreshes statistics.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How PostgreSQL automatically reclaims dead tuples and refreshes statistics. |
...
Think of autovacuum as a stateful database service whose data, WAL, locks, plans, and background workers must be observed together.
...
For autovacuum, the invariant is:

> The system must preserve the explicit state contract for how PostgreSQL automatically reclaims dead tuples and refreshes statistics, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on autovacuum. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| postgres-administration ladders | Use this chapter before opening project-specific placeholders that rely on autovacuum. |
...
You are ready to implement an exercise using autovacuum when you can:
...
1. Which state is made explicit by autovacuum that would otherwise be hidden?
