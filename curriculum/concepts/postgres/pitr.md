...
How point-in-time recovery combines a base backup with WAL replay.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How point-in-time recovery combines a base backup with WAL replay. |
...
Think of pITR as a stateful database service whose data, WAL, locks, plans, and background workers must be observed together.
...
For pITR, the invariant is:

> The system must preserve the explicit state contract for how point-in-time recovery combines a base backup with WAL replay, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on pITR. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| postgres-administration ladders | Use this chapter before opening project-specific placeholders that rely on pITR. |
...
You are ready to implement an exercise using pITR when you can:
...
1. Which state is made explicit by pITR that would otherwise be hidden?
