...
How the kernel decides who runs next and interrupts code that runs too long.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How the kernel decides who runs next and interrupts code that runs too long. |
...
Think of scheduling and preemption as a privileged state machine that admits requests through narrow, checked boundaries.
...
For scheduling and preemption, the invariant is:

> The system must preserve the explicit state contract for how the kernel decides who runs next and interrupts code that runs too long, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on scheduling and preemption. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| os-kernel-labs and systems-programming ladders | Use this chapter before opening project-specific placeholders that rely on scheduling and preemption. |
...
You are ready to implement an exercise using scheduling and preemption when you can:
...
1. Which state is made explicit by scheduling and preemption that would otherwise be hidden?
