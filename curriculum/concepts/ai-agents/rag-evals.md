...
How retrieval and generation quality are measured separately and together.
...
| Term | Meaning |
| --- | --- |
| Concept focus | How retrieval and generation quality are measured separately and together. |
...
Think of rAG evals as a probabilistic application pipeline with explicit evidence, actions, policies, and measurement.
...
For rAG evals, the invariant is:

> The system must preserve the explicit state contract for how retrieval and generation quality are measured separately and together, even when inputs are malformed, operations are retried, work is concurrent, or failure happens halfway through.
...
Imagine you are implementing a lab that depends on rAG evals. Start with one operation, one state variable, and one failure path.
...
| --- | --- |
| ai-rag-agents and LLM application ladders | Use this chapter before opening project-specific placeholders that rely on rAG evals. |
...
You are ready to implement an exercise using rAG evals when you can:
...
1. Which state is made explicit by rAG evals that would otherwise be hidden?
