# Exercise 003: Rate-Limited API Gateway Planning and Ordering

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Rate-Limited API Gateway**. The work is centered on `plan_rate_limited_api_gateway_allow_requests`, not on a generic scaffold. You will implement behavior for **api request** moving through **token bucket** while preserving the project invariant.

The implementation target is: Implement `plan_rate_limited_api_gateway_allow_requests` so Rate-Limited API Gateway can produce a deterministic allow request plan from desired and observed token bucket state.
...
The same desired and observed token bucket states must always produce the same minimal ordered allow request plan.

## Tiny Example

If `token-bucket-primary` is stale, `token-bucket-old` is extra, and `token-bucket-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_rate_limited_api_gateway_allow_requests` own?
...
Implement `plan_rate_limited_api_gateway_allow_requests` so Rate-Limited API Gateway can produce a deterministic allow request plan from desired and observed token bucket state.
