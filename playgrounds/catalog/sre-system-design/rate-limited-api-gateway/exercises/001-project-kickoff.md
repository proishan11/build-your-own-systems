# Exercise 001: Rate-Limited API Gateway First Implementation Lab

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This first lab turns **Rate-Limited API Gateway** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Implement per-tenant fixed-window rate limiting.
...
After each public operation, the component must preserve this contract: track counts per tenant and window; allow requests under limit; reject requests over limit. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Implement per-tenant fixed-window rate limiting.
...
- track counts per tenant and window
- allow requests under limit
- reject requests over limit
- reset counts in new windows
