# Exercise 005: Observability Stack Integration Scenario

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Observability Stack**. The work is centered on `run_observability_stack_scenario`, not on a generic scaffold. You will implement behavior for **trace span** moving through **signal pipeline** while preserving the project invariant.

The implementation target is: Implement `run_observability_stack_scenario` so Observability Stack has a deterministic integration simulation with state, `spans_ingested`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose signal pipeline state and spans_ingested metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `signal pipeline=ready`, recording `spans_ingested=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_observability_stack_scenario` own?
...
Implement `run_observability_stack_scenario` so Observability Stack has a deterministic integration simulation with state, `spans_ingested`, failures, recoveries, and invariant reporting.
...
- track `spans_ingested`, failures, and recoveries
