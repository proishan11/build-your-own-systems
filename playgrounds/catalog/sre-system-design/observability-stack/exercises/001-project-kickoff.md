# Exercise 001: Observability Stack Core Mechanism

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Observability Stack**. The work is centered on `ingest_telemetry_trace_span`, not on a generic scaffold. You will implement behavior for **trace span** moving through **signal pipeline** while preserving the project invariant.

The implementation target is: Implement `ingest_telemetry_trace_span` so Observability Stack has a concrete implementation boundary for trace span requests before they touch signal pipeline.
...
Only valid trace span requests may become ingest telemetry operations against signal pipeline; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `trace-span-001`, kind `ingest telemetry`, and target `signal pipeline` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `ingest_telemetry_trace_span` own?
...
Implement `ingest_telemetry_trace_span` so Observability Stack has a concrete implementation boundary for trace span requests before they touch signal pipeline.
...
- build valid trace span requests into a stable project operation
...
- Treat `cardinality explosion` as the kind of bad input that must never reach mutation code.
