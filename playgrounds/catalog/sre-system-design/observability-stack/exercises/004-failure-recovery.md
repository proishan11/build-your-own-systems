# Exercise 004: Observability Stack Failure and Recovery

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Observability Stack**. The work is centered on `recover_observability_stack_cardinality_explosion`, not on a generic scaffold. You will implement behavior for **trace span** moving through **signal pipeline** while preserving the project invariant.

The implementation target is: Implement `recover_observability_stack_cardinality_explosion` so Observability Stack handles transient failures, permanent `cardinality explosion` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for signal pipeline must never turn a permanent cardinality explosion into an unsafe retry.
...
1. What state does `recover_observability_stack_cardinality_explosion` own?
...
Implement `recover_observability_stack_cardinality_explosion` so Observability Stack handles transient failures, permanent `cardinality explosion` cases, and exhausted retry budgets explicitly.
...
- fail permanent `cardinality explosion` reports immediately
