# Exercise 003: Feature Flag and Rollout System Planning and Ordering

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Feature Flag and Rollout System**. The work is centered on `plan_feature_flag_and_rollout_system_evaluate_flags`, not on a generic scaffold. You will implement behavior for **flag evaluation** moving through **rollout rule** while preserving the project invariant.

The implementation target is: Implement `plan_feature_flag_and_rollout_system_evaluate_flags` so Feature Flag and Rollout System can produce a deterministic evaluate flag plan from desired and observed rollout rule state.
...
The same desired and observed rollout rule states must always produce the same minimal ordered evaluate flag plan.

## Tiny Example

If `rollout-rule-primary` is stale, `rollout-rule-old` is extra, and `rollout-rule-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_feature_flag_and_rollout_system_evaluate_flags` own?
...
Implement `plan_feature_flag_and_rollout_system_evaluate_flags` so Feature Flag and Rollout System can produce a deterministic evaluate flag plan from desired and observed rollout rule state.
