# Exercise 005: Dynamic Routing Simulator Integration Scenario

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Dynamic Routing Simulator**. The work is centered on `run_dynamic_routing_simulator_scenario`, not on a generic scaffold. You will implement behavior for **route advertisement** moving through **neighbor table** while preserving the project invariant.

The implementation target is: Implement `run_dynamic_routing_simulator_scenario` so Dynamic Routing Simulator has a deterministic integration simulation with state, `best_routes`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose neighbor table state and best_routes metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `neighbor table=ready`, recording `best_routes=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_dynamic_routing_simulator_scenario` own?
...
Implement `run_dynamic_routing_simulator_scenario` so Dynamic Routing Simulator has a deterministic integration simulation with state, `best_routes`, failures, recoveries, and invariant reporting.
...
- track `best_routes`, failures, and recoveries
