# Exercise 004: Dynamic Routing Simulator Failure and Recovery

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Dynamic Routing Simulator**. The work is centered on `recover_dynamic_routing_simulator_routing_loop`, not on a generic scaffold. You will implement behavior for **route advertisement** moving through **neighbor table** while preserving the project invariant.

The implementation target is: Implement `recover_dynamic_routing_simulator_routing_loop` so Dynamic Routing Simulator handles transient failures, permanent `routing loop` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for neighbor table must never turn a permanent routing loop into an unsafe retry.
...
1. What state does `recover_dynamic_routing_simulator_routing_loop` own?
...
Implement `recover_dynamic_routing_simulator_routing_loop` so Dynamic Routing Simulator handles transient failures, permanent `routing loop` cases, and exhausted retry budgets explicitly.
...
- fail permanent `routing loop` reports immediately
