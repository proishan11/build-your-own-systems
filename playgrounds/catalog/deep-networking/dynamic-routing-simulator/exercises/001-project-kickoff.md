# Exercise 001: Dynamic Routing Simulator First Implementation Lab

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This first lab turns **Dynamic Routing Simulator** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Implement one round of distance-vector route relaxation.
...
After each public operation, the component must preserve this contract: store local link costs; accept neighbor advertisements; choose lowest-cost next hop. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Implement one round of distance-vector route relaxation.
...
- store local link costs
- accept neighbor advertisements
- choose lowest-cost next hop
- update routes when a better path appears
