# Exercise 002: Dynamic Routing Simulator State and Invariants

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Dynamic Routing Simulator**. The work is centered on `apply_dynamic_routing_simulator_route_advertisement_event`, not on a generic scaffold. You will implement behavior for **route advertisement** moving through **neighbor table** while preserving the project invariant.

The implementation target is: Implement `apply_dynamic_routing_simulator_route_advertisement_event` so Dynamic Routing Simulator can replay route advertisement events into neighbor table without duplicating or accepting stale state.
...
neighbor table must advance monotonically by version, and duplicate route advertisement events must be idempotent.
...
1. What state does `apply_dynamic_routing_simulator_route_advertisement_event` own?
...
Implement `apply_dynamic_routing_simulator_route_advertisement_event` so Dynamic Routing Simulator can replay route advertisement events into neighbor table without duplicating or accepting stale state.
