# Exercise 005: Reliable Transport From Scratch Integration Scenario

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Reliable Transport From Scratch**. The work is centered on `run_reliable_transport_from_scratch_scenario`, not on a generic scaffold. You will implement behavior for **transport segment** moving through **receive window** while preserving the project invariant.

The implementation target is: Implement `run_reliable_transport_from_scratch_scenario` so Reliable Transport From Scratch has a deterministic integration simulation with state, `bytes_delivered`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose receive window state and bytes_delivered metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `receive window=ready`, recording `bytes_delivered=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_reliable_transport_from_scratch_scenario` own?
...
Implement `run_reliable_transport_from_scratch_scenario` so Reliable Transport From Scratch has a deterministic integration simulation with state, `bytes_delivered`, failures, recoveries, and invariant reporting.
...
- track `bytes_delivered`, failures, and recoveries
