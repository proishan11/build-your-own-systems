# Exercise 001: Reliable Transport From Scratch Core Mechanism

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Reliable Transport From Scratch**. The work is centered on `ack_segment_transport_segment`, not on a generic scaffold. You will implement behavior for **transport segment** moving through **receive window** while preserving the project invariant.

The implementation target is: Implement `ack_segment_transport_segment` so Reliable Transport From Scratch has a concrete implementation boundary for transport segment requests before they touch receive window.
...
Only valid transport segment requests may become ack segment operations against receive window; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `transport-segment-001`, kind `ack segment`, and target `receive window` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `ack_segment_transport_segment` own?
...
Implement `ack_segment_transport_segment` so Reliable Transport From Scratch has a concrete implementation boundary for transport segment requests before they touch receive window.
...
- build valid transport segment requests into a stable project operation
...
- Treat `out-of-order segment` as the kind of bad input that must never reach mutation code.
