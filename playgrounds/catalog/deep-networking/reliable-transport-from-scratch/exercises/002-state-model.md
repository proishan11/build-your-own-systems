# Exercise 002: Reliable Transport From Scratch State and Invariants

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Reliable Transport From Scratch**. The work is centered on `apply_reliable_transport_from_scratch_transport_segment_event`, not on a generic scaffold. You will implement behavior for **transport segment** moving through **receive window** while preserving the project invariant.

The implementation target is: Implement `apply_reliable_transport_from_scratch_transport_segment_event` so Reliable Transport From Scratch can replay transport segment events into receive window without duplicating or accepting stale state.
...
receive window must advance monotonically by version, and duplicate transport segment events must be idempotent.
...
1. What state does `apply_reliable_transport_from_scratch_transport_segment_event` own?
...
Implement `apply_reliable_transport_from_scratch_transport_segment_event` so Reliable Transport From Scratch can replay transport segment events into receive window without duplicating or accepting stale state.
