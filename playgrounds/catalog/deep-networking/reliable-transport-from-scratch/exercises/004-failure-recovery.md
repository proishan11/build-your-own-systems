# Exercise 004: Reliable Transport From Scratch Failure and Recovery

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Reliable Transport From Scratch**. The work is centered on `recover_reliable_transport_from_scratch_out_of_order_segment`, not on a generic scaffold. You will implement behavior for **transport segment** moving through **receive window** while preserving the project invariant.

The implementation target is: Implement `recover_reliable_transport_from_scratch_out_of_order_segment` so Reliable Transport From Scratch handles transient failures, permanent `out-of-order segment` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for receive window must never turn a permanent out-of-order segment into an unsafe retry.
...
1. What state does `recover_reliable_transport_from_scratch_out_of_order_segment` own?
...
Implement `recover_reliable_transport_from_scratch_out_of_order_segment` so Reliable Transport From Scratch handles transient failures, permanent `out-of-order segment` cases, and exhausted retry budgets explicitly.
...
- fail permanent `out-of-order segment` reports immediately
