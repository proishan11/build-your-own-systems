# Exercise 003: IP Router Lab Planning and Ordering

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **IP Router Lab**. The work is centered on `plan_ip_router_lab_forward_packets`, not on a generic scaffold. You will implement behavior for **ip packet** moving through **routing table** while preserving the project invariant.

The implementation target is: Implement `plan_ip_router_lab_forward_packets` so IP Router Lab can produce a deterministic forward packet plan from desired and observed routing table state.
...
The same desired and observed routing table states must always produce the same minimal ordered forward packet plan.

## Tiny Example

If `routing-table-primary` is stale, `routing-table-old` is extra, and `routing-table-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_ip_router_lab_forward_packets` own?
...
Implement `plan_ip_router_lab_forward_packets` so IP Router Lab can produce a deterministic forward packet plan from desired and observed routing table state.
