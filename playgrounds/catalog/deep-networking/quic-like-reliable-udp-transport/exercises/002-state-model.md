# Exercise 002: QUIC-Like Reliable UDP Transport State and Invariants

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **QUIC-Like Reliable UDP Transport**. The work is centered on `apply_quic_like_reliable_udp_transport_quic_frame_event`, not on a generic scaffold. You will implement behavior for **quic frame** moving through **connection state** while preserving the project invariant.

The implementation target is: Implement `apply_quic_like_reliable_udp_transport_quic_frame_event` so QUIC-Like Reliable UDP Transport can replay quic frame events into connection state without duplicating or accepting stale state.
...
connection state must advance monotonically by version, and duplicate quic frame events must be idempotent.
...
1. What state does `apply_quic_like_reliable_udp_transport_quic_frame_event` own?
...
Implement `apply_quic_like_reliable_udp_transport_quic_frame_event` so QUIC-Like Reliable UDP Transport can replay quic frame events into connection state without duplicating or accepting stale state.
