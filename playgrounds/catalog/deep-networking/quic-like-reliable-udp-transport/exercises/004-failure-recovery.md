# Exercise 004: QUIC-Like Reliable UDP Transport Failure and Recovery

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **QUIC-Like Reliable UDP Transport**. The work is centered on `recover_quic_like_reliable_udp_transport_lost_packet`, not on a generic scaffold. You will implement behavior for **quic frame** moving through **connection state** while preserving the project invariant.

The implementation target is: Implement `recover_quic_like_reliable_udp_transport_lost_packet` so QUIC-Like Reliable UDP Transport handles transient failures, permanent `lost packet` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for connection state must never turn a permanent lost packet into an unsafe retry.
...
1. What state does `recover_quic_like_reliable_udp_transport_lost_packet` own?
...
Implement `recover_quic_like_reliable_udp_transport_lost_packet` so QUIC-Like Reliable UDP Transport handles transient failures, permanent `lost packet` cases, and exhausted retry budgets explicitly.
...
- fail permanent `lost packet` reports immediately
