# Exercise 003: GraphRAG Knowledge System Planning and Ordering

Shared concept chapter: [rag-agents-and-tool-use.md](../../../../../curriculum/concepts/ai-agents/rag-agents-and-tool-use.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **GraphRAG Knowledge System**. The work is centered on `plan_graphrag_knowledge_system_link_entitys`, not on a generic scaffold. You will implement behavior for **entity mention** moving through **knowledge graph** while preserving the project invariant.

The implementation target is: Implement `plan_graphrag_knowledge_system_link_entitys` so GraphRAG Knowledge System can produce a deterministic link entity plan from desired and observed knowledge graph state.
...
The same desired and observed knowledge graph states must always produce the same minimal ordered link entity plan.

## Tiny Example

If `knowledge-graph-primary` is stale, `knowledge-graph-old` is extra, and `knowledge-graph-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_graphrag_knowledge_system_link_entitys` own?
...
Implement `plan_graphrag_knowledge_system_link_entitys` so GraphRAG Knowledge System can produce a deterministic link entity plan from desired and observed knowledge graph state.
