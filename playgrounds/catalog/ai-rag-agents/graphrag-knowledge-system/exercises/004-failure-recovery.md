# Exercise 004: GraphRAG Knowledge System Failure and Recovery

Shared concept chapter: [rag-agents-and-tool-use.md](../../../../../curriculum/concepts/ai-agents/rag-agents-and-tool-use.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **GraphRAG Knowledge System**. The work is centered on `recover_graphrag_knowledge_system_duplicate_entity`, not on a generic scaffold. You will implement behavior for **entity mention** moving through **knowledge graph** while preserving the project invariant.

The implementation target is: Implement `recover_graphrag_knowledge_system_duplicate_entity` so GraphRAG Knowledge System handles transient failures, permanent `duplicate entity` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for knowledge graph must never turn a permanent duplicate entity into an unsafe retry.
...
1. What state does `recover_graphrag_knowledge_system_duplicate_entity` own?
...
Implement `recover_graphrag_knowledge_system_duplicate_entity` so GraphRAG Knowledge System handles transient failures, permanent `duplicate entity` cases, and exhausted retry budgets explicitly.
...
- fail permanent `duplicate entity` reports immediately
