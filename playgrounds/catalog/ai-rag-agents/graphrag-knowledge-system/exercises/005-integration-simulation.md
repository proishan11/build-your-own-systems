# Exercise 005: GraphRAG Knowledge System Integration Scenario

Shared concept chapter: [rag-agents-and-tool-use.md](../../../../../curriculum/concepts/ai-agents/rag-agents-and-tool-use.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **GraphRAG Knowledge System**. The work is centered on `run_graphrag_knowledge_system_scenario`, not on a generic scaffold. You will implement behavior for **entity mention** moving through **knowledge graph** while preserving the project invariant.

The implementation target is: Implement `run_graphrag_knowledge_system_scenario` so GraphRAG Knowledge System has a deterministic integration simulation with state, `communities_found`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose knowledge graph state and communities_found metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `knowledge graph=ready`, recording `communities_found=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_graphrag_knowledge_system_scenario` own?
...
Implement `run_graphrag_knowledge_system_scenario` so GraphRAG Knowledge System has a deterministic integration simulation with state, `communities_found`, failures, recoveries, and invariant reporting.
...
- track `communities_found`, failures, and recoveries
