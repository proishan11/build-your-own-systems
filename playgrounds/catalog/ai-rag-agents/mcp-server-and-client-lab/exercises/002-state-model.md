# Exercise 002: MCP Server and Client Lab State and Invariants

Shared concept chapter: [rag-agents-and-tool-use.md](../../../../../curriculum/concepts/ai-agents/rag-agents-and-tool-use.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **MCP Server and Client Lab**. The work is centered on `apply_mcp_server_and_client_lab_mcp_request_event`, not on a generic scaffold. You will implement behavior for **mcp request** moving through **tool registry** while preserving the project invariant.

The implementation target is: Implement `apply_mcp_server_and_client_lab_mcp_request_event` so MCP Server and Client Lab can replay mcp request events into tool registry without duplicating or accepting stale state.
...
tool registry must advance monotonically by version, and duplicate mcp request events must be idempotent.
...
1. What state does `apply_mcp_server_and_client_lab_mcp_request_event` own?
...
Implement `apply_mcp_server_and_client_lab_mcp_request_event` so MCP Server and Client Lab can replay mcp request events into tool registry without duplicating or accepting stale state.
