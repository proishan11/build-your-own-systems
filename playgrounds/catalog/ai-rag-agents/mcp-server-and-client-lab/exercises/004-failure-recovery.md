# Exercise 004: MCP Server and Client Lab Failure and Recovery

Shared concept chapter: [rag-agents-and-tool-use.md](../../../../../curriculum/concepts/ai-agents/rag-agents-and-tool-use.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **MCP Server and Client Lab**. The work is centered on `recover_mcp_server_and_client_lab_invalid_json_rpc_id`, not on a generic scaffold. You will implement behavior for **mcp request** moving through **tool registry** while preserving the project invariant.

The implementation target is: Implement `recover_mcp_server_and_client_lab_invalid_json_rpc_id` so MCP Server and Client Lab handles transient failures, permanent `invalid json-rpc id` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for tool registry must never turn a permanent invalid json-rpc id into an unsafe retry.
...
1. What state does `recover_mcp_server_and_client_lab_invalid_json_rpc_id` own?
...
Implement `recover_mcp_server_and_client_lab_invalid_json_rpc_id` so MCP Server and Client Lab handles transient failures, permanent `invalid json-rpc id` cases, and exhausted retry budgets explicitly.
...
- fail permanent `invalid json-rpc id` reports immediately
