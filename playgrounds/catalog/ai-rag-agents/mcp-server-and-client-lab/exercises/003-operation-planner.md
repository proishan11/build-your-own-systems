# Exercise 003: MCP Server and Client Lab Planning and Ordering

Shared concept chapter: [rag-agents-and-tool-use.md](../../../../../curriculum/concepts/ai-agents/rag-agents-and-tool-use.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **MCP Server and Client Lab**. The work is centered on `plan_mcp_server_and_client_lab_dispatch_tools`, not on a generic scaffold. You will implement behavior for **mcp request** moving through **tool registry** while preserving the project invariant.

The implementation target is: Implement `plan_mcp_server_and_client_lab_dispatch_tools` so MCP Server and Client Lab can produce a deterministic dispatch tool plan from desired and observed tool registry state.
...
The same desired and observed tool registry states must always produce the same minimal ordered dispatch tool plan.

## Tiny Example

If `tool-registry-primary` is stale, `tool-registry-old` is extra, and `tool-registry-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_mcp_server_and_client_lab_dispatch_tools` own?
...
Implement `plan_mcp_server_and_client_lab_dispatch_tools` so MCP Server and Client Lab can produce a deterministic dispatch tool plan from desired and observed tool registry state.
