# Exercise 002: RAG From Scratch State and Invariants

Shared concept chapter: [rag-agents-and-tool-use.md](../../../../../curriculum/concepts/ai-agents/rag-agents-and-tool-use.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **RAG From Scratch**. The work is centered on `apply_rag_from_scratch_document_chunk_event`, not on a generic scaffold. You will implement behavior for **document chunk** moving through **vector index** while preserving the project invariant.

The implementation target is: Implement `apply_rag_from_scratch_document_chunk_event` so RAG From Scratch can replay document chunk events into vector index without duplicating or accepting stale state.
...
vector index must advance monotonically by version, and duplicate document chunk events must be idempotent.
...
1. What state does `apply_rag_from_scratch_document_chunk_event` own?
...
Implement `apply_rag_from_scratch_document_chunk_event` so RAG From Scratch can replay document chunk events into vector index without duplicating or accepting stale state.
