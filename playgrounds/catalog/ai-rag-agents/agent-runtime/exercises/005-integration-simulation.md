# Exercise 005: Agent Runtime Integration Scenario

Shared concept chapter: [rag-agents-and-tool-use.md](../../../../../curriculum/concepts/ai-agents/rag-agents-and-tool-use.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Agent Runtime**. The work is centered on `run_agent_runtime_scenario`, not on a generic scaffold. You will implement behavior for **agent step** moving through **execution trace** while preserving the project invariant.

The implementation target is: Implement `run_agent_runtime_scenario` so Agent Runtime has a deterministic integration simulation with state, `steps_executed`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose execution trace state and steps_executed metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `execution trace=ready`, recording `steps_executed=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_agent_runtime_scenario` own?
...
Implement `run_agent_runtime_scenario` so Agent Runtime has a deterministic integration simulation with state, `steps_executed`, failures, recoveries, and invariant reporting.
...
- track `steps_executed`, failures, and recoveries
