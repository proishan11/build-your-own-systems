# Exercise 003: Agent Runtime Planning and Ordering

Shared concept chapter: [rag-agents-and-tool-use.md](../../../../../curriculum/concepts/ai-agents/rag-agents-and-tool-use.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Agent Runtime**. The work is centered on `plan_agent_runtime_choose_tools`, not on a generic scaffold. You will implement behavior for **agent step** moving through **execution trace** while preserving the project invariant.

The implementation target is: Implement `plan_agent_runtime_choose_tools` so Agent Runtime can produce a deterministic choose tool plan from desired and observed execution trace state.
...
The same desired and observed execution trace states must always produce the same minimal ordered choose tool plan.

## Tiny Example

If `execution-trace-primary` is stale, `execution-trace-old` is extra, and `execution-trace-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_agent_runtime_choose_tools` own?
...
Implement `plan_agent_runtime_choose_tools` so Agent Runtime can produce a deterministic choose tool plan from desired and observed execution trace state.
