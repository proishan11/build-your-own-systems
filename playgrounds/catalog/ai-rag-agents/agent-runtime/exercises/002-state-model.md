# Exercise 002: Agent Runtime State and Invariants

Shared concept chapter: [rag-agents-and-tool-use.md](../../../../../curriculum/concepts/ai-agents/rag-agents-and-tool-use.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Agent Runtime**. The work is centered on `apply_agent_runtime_agent_step_event`, not on a generic scaffold. You will implement behavior for **agent step** moving through **execution trace** while preserving the project invariant.

The implementation target is: Implement `apply_agent_runtime_agent_step_event` so Agent Runtime can replay agent step events into execution trace without duplicating or accepting stale state.
...
execution trace must advance monotonically by version, and duplicate agent step events must be idempotent.
...
1. What state does `apply_agent_runtime_agent_step_event` own?
...
Implement `apply_agent_runtime_agent_step_event` so Agent Runtime can replay agent step events into execution trace without duplicating or accepting stale state.
