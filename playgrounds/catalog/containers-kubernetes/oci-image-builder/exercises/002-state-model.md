# Exercise 002: OCI Image Builder State and Invariants
...
This exercise is a project-specific implementation milestone for **OCI Image Builder**. The work is centered on `apply_oci_image_builder_image_layer_event`, not on a generic scaffold. You will implement behavior for **image layer** moving through **image manifest** while preserving the project invariant.

The implementation target is: Implement `apply_oci_image_builder_image_layer_event` so OCI Image Builder can replay image layer events into image manifest without duplicating or accepting stale state.
...
image manifest must advance monotonically by version, and duplicate image layer events must be idempotent.
...
1. What state does `apply_oci_image_builder_image_layer_event` own?
...
Implement `apply_oci_image_builder_image_layer_event` so OCI Image Builder can replay image layer events into image manifest without duplicating or accepting stale state.
