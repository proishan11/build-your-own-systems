# Exercise 005: OCI Image Builder Integration Scenario
...
This exercise is a project-specific implementation milestone for **OCI Image Builder**. The work is centered on `run_oci_image_builder_scenario`, not on a generic scaffold. You will implement behavior for **image layer** moving through **image manifest** while preserving the project invariant.

The implementation target is: Implement `run_oci_image_builder_scenario` so OCI Image Builder has a deterministic integration simulation with state, `layers_reused`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose image manifest state and layers_reused metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `image manifest=ready`, recording `layers_reused=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_oci_image_builder_scenario` own?
...
Implement `run_oci_image_builder_scenario` so OCI Image Builder has a deterministic integration simulation with state, `layers_reused`, failures, recoveries, and invariant reporting.
...
- track `layers_reused`, failures, and recoveries
