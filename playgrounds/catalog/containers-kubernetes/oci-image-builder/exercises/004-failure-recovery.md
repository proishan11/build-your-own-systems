# Exercise 004: OCI Image Builder Failure and Recovery
...
This exercise is a project-specific implementation milestone for **OCI Image Builder**. The work is centered on `recover_oci_image_builder_non_reproducible_layer`, not on a generic scaffold. You will implement behavior for **image layer** moving through **image manifest** while preserving the project invariant.

The implementation target is: Implement `recover_oci_image_builder_non_reproducible_layer` so OCI Image Builder handles transient failures, permanent `non-reproducible layer` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for image manifest must never turn a permanent non-reproducible layer into an unsafe retry.
...
1. What state does `recover_oci_image_builder_non_reproducible_layer` own?
...
Implement `recover_oci_image_builder_non_reproducible_layer` so OCI Image Builder handles transient failures, permanent `non-reproducible layer` cases, and exhausted retry budgets explicitly.
...
- fail permanent `non-reproducible layer` reports immediately
