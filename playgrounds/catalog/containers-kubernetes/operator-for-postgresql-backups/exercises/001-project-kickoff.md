# Exercise 001: Operator for PostgreSQL Backups First Implementation Lab
...
This first lab turns **Operator for PostgreSQL Backups** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Implement a reconciliation decision for desired replicas.
...
After each public operation, the component must preserve this contract: compare desired and observed replica counts; return create/delete/noop actions; update status message. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Implement a reconciliation decision for desired replicas.
...
- compare desired and observed replica counts
- return create/delete/noop actions
- update status message
- make repeated reconciliation idempotent
