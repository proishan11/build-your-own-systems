# Exercise 002: Operator for PostgreSQL Backups State and Invariants
...
This exercise is a project-specific implementation milestone for **Operator for PostgreSQL Backups**. The work is centered on `apply_operator_for_postgresql_backups_backup_custom_resource_event`, not on a generic scaffold. You will implement behavior for **backup custom resource** moving through **backup schedule** while preserving the project invariant.

The implementation target is: Implement `apply_operator_for_postgresql_backups_backup_custom_resource_event` so Operator for PostgreSQL Backups can replay backup custom resource events into backup schedule without duplicating or accepting stale state.
...
backup schedule must advance monotonically by version, and duplicate backup custom resource events must be idempotent.
...
1. What state does `apply_operator_for_postgresql_backups_backup_custom_resource_event` own?
...
Implement `apply_operator_for_postgresql_backups_backup_custom_resource_event` so Operator for PostgreSQL Backups can replay backup custom resource events into backup schedule without duplicating or accepting stale state.
