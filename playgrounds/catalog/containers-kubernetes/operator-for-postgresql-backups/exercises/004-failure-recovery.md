# Exercise 004: Operator for PostgreSQL Backups Failure and Recovery
...
This exercise is a project-specific implementation milestone for **Operator for PostgreSQL Backups**. The work is centered on `recover_operator_for_postgresql_backups_missing_wal_archive`, not on a generic scaffold. You will implement behavior for **backup custom resource** moving through **backup schedule** while preserving the project invariant.

The implementation target is: Implement `recover_operator_for_postgresql_backups_missing_wal_archive` so Operator for PostgreSQL Backups handles transient failures, permanent `missing wal archive` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for backup schedule must never turn a permanent missing wal archive into an unsafe retry.
...
1. What state does `recover_operator_for_postgresql_backups_missing_wal_archive` own?
...
Implement `recover_operator_for_postgresql_backups_missing_wal_archive` so Operator for PostgreSQL Backups handles transient failures, permanent `missing wal archive` cases, and exhausted retry budgets explicitly.
...
- fail permanent `missing wal archive` reports immediately
