# Exercise 001: Kubernetes Controller From Scratch Core Mechanism
...
This exercise is a project-specific implementation milestone for **Kubernetes Controller From Scratch**. The work is centered on `reconcile_object_custom_resource`, not on a generic scaffold. You will implement behavior for **custom resource** moving through **reconcile state** while preserving the project invariant.

The implementation target is: Implement `reconcile_object_custom_resource` so Kubernetes Controller From Scratch has a concrete implementation boundary for custom resource requests before they touch reconcile state.
...
Only valid custom resource requests may become reconcile object operations against reconcile state; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `custom-resource-001`, kind `reconcile object`, and target `reconcile state` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `reconcile_object_custom_resource` own?
...
Implement `reconcile_object_custom_resource` so Kubernetes Controller From Scratch has a concrete implementation boundary for custom resource requests before they touch reconcile state.
...
- build valid custom resource requests into a stable project operation
...
- Treat `stale observed generation` as the kind of bad input that must never reach mutation code.
