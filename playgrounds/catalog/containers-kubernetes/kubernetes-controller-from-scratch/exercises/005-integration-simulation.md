# Exercise 005: Kubernetes Controller From Scratch Integration Scenario
...
This exercise is a project-specific implementation milestone for **Kubernetes Controller From Scratch**. The work is centered on `run_kubernetes_controller_from_scratch_scenario`, not on a generic scaffold. You will implement behavior for **custom resource** moving through **reconcile state** while preserving the project invariant.

The implementation target is: Implement `run_kubernetes_controller_from_scratch_scenario` so Kubernetes Controller From Scratch has a deterministic integration simulation with state, `reconciles_total`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose reconcile state state and reconciles_total metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `reconcile state=ready`, recording `reconciles_total=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_kubernetes_controller_from_scratch_scenario` own?
...
Implement `run_kubernetes_controller_from_scratch_scenario` so Kubernetes Controller From Scratch has a deterministic integration simulation with state, `reconciles_total`, failures, recoveries, and invariant reporting.
...
- track `reconciles_total`, failures, and recoveries
