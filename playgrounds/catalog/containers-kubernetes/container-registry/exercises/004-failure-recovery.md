# Exercise 004: Container Registry Failure and Recovery
...
This exercise is a project-specific implementation milestone for **Container Registry**. The work is centered on `recover_container_registry_digest_mismatch`, not on a generic scaffold. You will implement behavior for **image blob** moving through **blob store** while preserving the project invariant.

The implementation target is: Implement `recover_container_registry_digest_mismatch` so Container Registry handles transient failures, permanent `digest mismatch` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for blob store must never turn a permanent digest mismatch into an unsafe retry.
...
1. What state does `recover_container_registry_digest_mismatch` own?
...
Implement `recover_container_registry_digest_mismatch` so Container Registry handles transient failures, permanent `digest mismatch` cases, and exhausted retry budgets explicitly.
...
- fail permanent `digest mismatch` reports immediately
