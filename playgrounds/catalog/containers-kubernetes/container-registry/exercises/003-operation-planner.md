# Exercise 003: Container Registry Planning and Ordering
...
This exercise is a project-specific implementation milestone for **Container Registry**. The work is centered on `plan_container_registry_serve_manifests`, not on a generic scaffold. You will implement behavior for **image blob** moving through **blob store** while preserving the project invariant.

The implementation target is: Implement `plan_container_registry_serve_manifests` so Container Registry can produce a deterministic serve manifest plan from desired and observed blob store state.
...
The same desired and observed blob store states must always produce the same minimal ordered serve manifest plan.

## Tiny Example

If `blob-store-primary` is stale, `blob-store-old` is extra, and `blob-store-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_container_registry_serve_manifests` own?
...
Implement `plan_container_registry_serve_manifests` so Container Registry can produce a deterministic serve manifest plan from desired and observed blob store state.
