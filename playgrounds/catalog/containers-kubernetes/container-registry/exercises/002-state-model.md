# Exercise 002: Container Registry State and Invariants
...
This exercise is a project-specific implementation milestone for **Container Registry**. The work is centered on `apply_container_registry_image_blob_event`, not on a generic scaffold. You will implement behavior for **image blob** moving through **blob store** while preserving the project invariant.

The implementation target is: Implement `apply_container_registry_image_blob_event` so Container Registry can replay image blob events into blob store without duplicating or accepting stale state.
...
blob store must advance monotonically by version, and duplicate image blob events must be idempotent.
...
1. What state does `apply_container_registry_image_blob_event` own?
...
Implement `apply_container_registry_image_blob_event` so Container Registry can replay image blob events into blob store without duplicating or accepting stale state.
