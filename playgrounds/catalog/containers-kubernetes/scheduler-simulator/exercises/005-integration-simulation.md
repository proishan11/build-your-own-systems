# Exercise 005: Scheduler Simulator Integration Scenario
...
This exercise is a project-specific implementation milestone for **Scheduler Simulator**. The work is centered on `run_scheduler_simulator_scenario`, not on a generic scaffold. You will implement behavior for **pod placement** moving through **node inventory** while preserving the project invariant.

The implementation target is: Implement `run_scheduler_simulator_scenario` so Scheduler Simulator has a deterministic integration simulation with state, `pods_scheduled`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose node inventory state and pods_scheduled metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `node inventory=ready`, recording `pods_scheduled=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_scheduler_simulator_scenario` own?
...
Implement `run_scheduler_simulator_scenario` so Scheduler Simulator has a deterministic integration simulation with state, `pods_scheduled`, failures, recoveries, and invariant reporting.
...
- track `pods_scheduled`, failures, and recoveries
