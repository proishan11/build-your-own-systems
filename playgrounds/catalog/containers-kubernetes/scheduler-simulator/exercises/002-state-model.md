# Exercise 002: Scheduler Simulator State and Invariants
...
This exercise is a project-specific implementation milestone for **Scheduler Simulator**. The work is centered on `apply_scheduler_simulator_pod_placement_event`, not on a generic scaffold. You will implement behavior for **pod placement** moving through **node inventory** while preserving the project invariant.

The implementation target is: Implement `apply_scheduler_simulator_pod_placement_event` so Scheduler Simulator can replay pod placement events into node inventory without duplicating or accepting stale state.
...
node inventory must advance monotonically by version, and duplicate pod placement events must be idempotent.
...
1. What state does `apply_scheduler_simulator_pod_placement_event` own?
...
Implement `apply_scheduler_simulator_pod_placement_event` so Scheduler Simulator can replay pod placement events into node inventory without duplicating or accepting stale state.
