# Exercise 003: Scheduler Simulator Planning and Ordering
...
This exercise is a project-specific implementation milestone for **Scheduler Simulator**. The work is centered on `plan_scheduler_simulator_bind_pods`, not on a generic scaffold. You will implement behavior for **pod placement** moving through **node inventory** while preserving the project invariant.

The implementation target is: Implement `plan_scheduler_simulator_bind_pods` so Scheduler Simulator can produce a deterministic bind pod plan from desired and observed node inventory state.
...
The same desired and observed node inventory states must always produce the same minimal ordered bind pod plan.

## Tiny Example

If `node-inventory-primary` is stale, `node-inventory-old` is extra, and `node-inventory-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_scheduler_simulator_bind_pods` own?
...
Implement `plan_scheduler_simulator_bind_pods` so Scheduler Simulator can produce a deterministic bind pod plan from desired and observed node inventory state.
