# Exercise 004: Admission Controller Failure and Recovery
...
This exercise is a project-specific implementation milestone for **Admission Controller**. The work is centered on `recover_admission_controller_policy_bypass`, not on a generic scaffold. You will implement behavior for **admission request** moving through **policy set** while preserving the project invariant.

The implementation target is: Implement `recover_admission_controller_policy_bypass` so Admission Controller handles transient failures, permanent `policy bypass` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for policy set must never turn a permanent policy bypass into an unsafe retry.
...
1. What state does `recover_admission_controller_policy_bypass` own?
...
Implement `recover_admission_controller_policy_bypass` so Admission Controller handles transient failures, permanent `policy bypass` cases, and exhausted retry budgets explicitly.
...
- fail permanent `policy bypass` reports immediately
