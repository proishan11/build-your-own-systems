# Exercise 002: Admission Controller State and Invariants
...
This exercise is a project-specific implementation milestone for **Admission Controller**. The work is centered on `apply_admission_controller_admission_request_event`, not on a generic scaffold. You will implement behavior for **admission request** moving through **policy set** while preserving the project invariant.

The implementation target is: Implement `apply_admission_controller_admission_request_event` so Admission Controller can replay admission request events into policy set without duplicating or accepting stale state.
...
policy set must advance monotonically by version, and duplicate admission request events must be idempotent.
...
1. What state does `apply_admission_controller_admission_request_event` own?
...
Implement `apply_admission_controller_admission_request_event` so Admission Controller can replay admission request events into policy set without duplicating or accepting stale state.
