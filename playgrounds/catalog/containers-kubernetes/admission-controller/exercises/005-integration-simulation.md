# Exercise 005: Admission Controller Integration Scenario
...
This exercise is a project-specific implementation milestone for **Admission Controller**. The work is centered on `run_admission_controller_scenario`, not on a generic scaffold. You will implement behavior for **admission request** moving through **policy set** while preserving the project invariant.

The implementation target is: Implement `run_admission_controller_scenario` so Admission Controller has a deterministic integration simulation with state, `requests_denied`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose policy set state and requests_denied metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `policy set=ready`, recording `requests_denied=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_admission_controller_scenario` own?
...
Implement `run_admission_controller_scenario` so Admission Controller has a deterministic integration simulation with state, `requests_denied`, failures, recoveries, and invariant reporting.
...
- track `requests_denied`, failures, and recoveries
