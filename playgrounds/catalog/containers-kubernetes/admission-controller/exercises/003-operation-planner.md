# Exercise 003: Admission Controller Planning and Ordering
...
This exercise is a project-specific implementation milestone for **Admission Controller**. The work is centered on `plan_admission_controller_admit_objects`, not on a generic scaffold. You will implement behavior for **admission request** moving through **policy set** while preserving the project invariant.

The implementation target is: Implement `plan_admission_controller_admit_objects` so Admission Controller can produce a deterministic admit object plan from desired and observed policy set state.
...
The same desired and observed policy set states must always produce the same minimal ordered admit object plan.

## Tiny Example

If `policy-set-primary` is stale, `policy-set-old` is extra, and `policy-set-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_admission_controller_admit_objects` own?
...
Implement `plan_admission_controller_admit_objects` so Admission Controller can produce a deterministic admit object plan from desired and observed policy set state.
