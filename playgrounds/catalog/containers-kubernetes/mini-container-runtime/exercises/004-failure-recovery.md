# Exercise 004: Mini Container Runtime Failure and Recovery
...
This exercise is a project-specific implementation milestone for **Mini Container Runtime**. The work is centered on `recover_mini_container_runtime_missing_isolation`, not on a generic scaffold. You will implement behavior for **container spec** moving through **namespace plan** while preserving the project invariant.

The implementation target is: Implement `recover_mini_container_runtime_missing_isolation` so Mini Container Runtime handles transient failures, permanent `missing isolation` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for namespace plan must never turn a permanent missing isolation into an unsafe retry.
...
1. What state does `recover_mini_container_runtime_missing_isolation` own?
...
Implement `recover_mini_container_runtime_missing_isolation` so Mini Container Runtime handles transient failures, permanent `missing isolation` cases, and exhausted retry budgets explicitly.
...
- fail permanent `missing isolation` reports immediately
