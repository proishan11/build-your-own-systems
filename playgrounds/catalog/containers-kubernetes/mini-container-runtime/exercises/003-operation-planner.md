# Exercise 003: Mini Container Runtime Planning and Ordering
...
This exercise is a project-specific implementation milestone for **Mini Container Runtime**. The work is centered on `plan_mini_container_runtime_start_containers`, not on a generic scaffold. You will implement behavior for **container spec** moving through **namespace plan** while preserving the project invariant.

The implementation target is: Implement `plan_mini_container_runtime_start_containers` so Mini Container Runtime can produce a deterministic start container plan from desired and observed namespace plan state.
...
The same desired and observed namespace plan states must always produce the same minimal ordered start container plan.

## Tiny Example

If `namespace-plan-primary` is stale, `namespace-plan-old` is extra, and `namespace-plan-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_mini_container_runtime_start_containers` own?
...
Implement `plan_mini_container_runtime_start_containers` so Mini Container Runtime can produce a deterministic start container plan from desired and observed namespace plan state.
