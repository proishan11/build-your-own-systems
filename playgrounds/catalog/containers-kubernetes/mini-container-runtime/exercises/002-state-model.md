# Exercise 002: Mini Container Runtime State and Invariants
...
This exercise is a project-specific implementation milestone for **Mini Container Runtime**. The work is centered on `apply_mini_container_runtime_container_spec_event`, not on a generic scaffold. You will implement behavior for **container spec** moving through **namespace plan** while preserving the project invariant.

The implementation target is: Implement `apply_mini_container_runtime_container_spec_event` so Mini Container Runtime can replay container spec events into namespace plan without duplicating or accepting stale state.
...
namespace plan must advance monotonically by version, and duplicate container spec events must be idempotent.
...
1. What state does `apply_mini_container_runtime_container_spec_event` own?
...
Implement `apply_mini_container_runtime_container_spec_event` so Mini Container Runtime can replay container spec events into namespace plan without duplicating or accepting stale state.
