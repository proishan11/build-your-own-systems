# Exercise 001: Memory Allocator Lab Core Mechanism

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Memory Allocator Lab**. The work is centered on `split_allocation_heap_allocation`, not on a generic scaffold. You will implement behavior for **heap allocation** moving through **free block** while preserving the project invariant.

The implementation target is: Implement `split_allocation_heap_allocation` so Memory Allocator Lab has a concrete implementation boundary for heap allocation requests before they touch free block.
...
Only valid heap allocation requests may become split allocation operations against free block; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `heap-allocation-001`, kind `split allocation`, and target `free block` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `split_allocation_heap_allocation` own?
...
Implement `split_allocation_heap_allocation` so Memory Allocator Lab has a concrete implementation boundary for heap allocation requests before they touch free block.
...
- build valid heap allocation requests into a stable project operation
...
- Treat `double free` as the kind of bad input that must never reach mutation code.
