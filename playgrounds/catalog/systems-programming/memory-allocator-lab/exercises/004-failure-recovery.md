# Exercise 004: Memory Allocator Lab Failure and Recovery

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Memory Allocator Lab**. The work is centered on `recover_memory_allocator_lab_double_free`, not on a generic scaffold. You will implement behavior for **heap allocation** moving through **free block** while preserving the project invariant.

The implementation target is: Implement `recover_memory_allocator_lab_double_free` so Memory Allocator Lab handles transient failures, permanent `double free` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for free block must never turn a permanent double free into an unsafe retry.
...
1. What state does `recover_memory_allocator_lab_double_free` own?
...
Implement `recover_memory_allocator_lab_double_free` so Memory Allocator Lab handles transient failures, permanent `double free` cases, and exhausted retry budgets explicitly.
...
- fail permanent `double free` reports immediately
