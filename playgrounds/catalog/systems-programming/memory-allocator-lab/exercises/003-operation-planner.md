# Exercise 003: Memory Allocator Lab Planning and Ordering

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Memory Allocator Lab**. The work is centered on `plan_memory_allocator_lab_split_allocations`, not on a generic scaffold. You will implement behavior for **heap allocation** moving through **free block** while preserving the project invariant.

The implementation target is: Implement `plan_memory_allocator_lab_split_allocations` so Memory Allocator Lab can produce a deterministic split allocation plan from desired and observed free block state.
...
The same desired and observed free block states must always produce the same minimal ordered split allocation plan.

## Tiny Example

If `free-block-primary` is stale, `free-block-old` is extra, and `free-block-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_memory_allocator_lab_split_allocations` own?
...
Implement `plan_memory_allocator_lab_split_allocations` so Memory Allocator Lab can produce a deterministic split allocation plan from desired and observed free block state.
