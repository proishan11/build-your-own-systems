# Exercise 002: Memory Allocator Lab State and Invariants

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Memory Allocator Lab**. The work is centered on `apply_memory_allocator_lab_heap_allocation_event`, not on a generic scaffold. You will implement behavior for **heap allocation** moving through **free block** while preserving the project invariant.

The implementation target is: Implement `apply_memory_allocator_lab_heap_allocation_event` so Memory Allocator Lab can replay heap allocation events into free block without duplicating or accepting stale state.
...
free block must advance monotonically by version, and duplicate heap allocation events must be idempotent.
...
1. What state does `apply_memory_allocator_lab_heap_allocation_event` own?
...
Implement `apply_memory_allocator_lab_heap_allocation_event` so Memory Allocator Lab can replay heap allocation events into free block without duplicating or accepting stale state.
