# Exercise 002: Mini Shell With Job Control State and Invariants

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Mini Shell With Job Control**. The work is centered on `apply_mini_shell_with_job_control_command_pipeline_event`, not on a generic scaffold. You will implement behavior for **command pipeline** moving through **process group** while preserving the project invariant.

The implementation target is: Implement `apply_mini_shell_with_job_control_command_pipeline_event` so Mini Shell With Job Control can replay command pipeline events into process group without duplicating or accepting stale state.
...
process group must advance monotonically by version, and duplicate command pipeline events must be idempotent.
...
1. What state does `apply_mini_shell_with_job_control_command_pipeline_event` own?
...
Implement `apply_mini_shell_with_job_control_command_pipeline_event` so Mini Shell With Job Control can replay command pipeline events into process group without duplicating or accepting stale state.
