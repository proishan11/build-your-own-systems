# Exercise 004: Mini Shell With Job Control Failure and Recovery

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Mini Shell With Job Control**. The work is centered on `recover_mini_shell_with_job_control_orphaned_background_job`, not on a generic scaffold. You will implement behavior for **command pipeline** moving through **process group** while preserving the project invariant.

The implementation target is: Implement `recover_mini_shell_with_job_control_orphaned_background_job` so Mini Shell With Job Control handles transient failures, permanent `orphaned background job` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for process group must never turn a permanent orphaned background job into an unsafe retry.
...
1. What state does `recover_mini_shell_with_job_control_orphaned_background_job` own?
...
Implement `recover_mini_shell_with_job_control_orphaned_background_job` so Mini Shell With Job Control handles transient failures, permanent `orphaned background job` cases, and exhausted retry budgets explicitly.
...
- fail permanent `orphaned background job` reports immediately
