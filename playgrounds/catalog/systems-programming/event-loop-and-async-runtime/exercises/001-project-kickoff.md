# Exercise 001: Event Loop and Async Runtime First Implementation Lab

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This first lab turns **Event Loop and Async Runtime** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Implement a deterministic timer queue for a tiny event loop.
...
After each public operation, the component must preserve this contract: schedule callbacks at logical times; run callbacks in time order; preserve insertion order for equal times. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Implement a deterministic timer queue for a tiny event loop.
...
- schedule callbacks at logical times
- run callbacks in time order
- preserve insertion order for equal times
- advance logical time as callbacks run
