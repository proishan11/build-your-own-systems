# Exercise 004: Query Optimizer Lab Failure and Recovery

Shared concept chapter: [write-ahead-logs.md](../../../../../curriculum/concepts/storage/write-ahead-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Query Optimizer Lab**. The work is centered on `recover_query_optimizer_lab_bad_cardinality_estimate`, not on a generic scaffold. You will implement behavior for **query predicate** moving through **plan space** while preserving the project invariant.

The implementation target is: Implement `recover_query_optimizer_lab_bad_cardinality_estimate` so Query Optimizer Lab handles transient failures, permanent `bad cardinality estimate` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for plan space must never turn a permanent bad cardinality estimate into an unsafe retry.
...
1. What state does `recover_query_optimizer_lab_bad_cardinality_estimate` own?
...
Implement `recover_query_optimizer_lab_bad_cardinality_estimate` so Query Optimizer Lab handles transient failures, permanent `bad cardinality estimate` cases, and exhausted retry budgets explicitly.
...
- fail permanent `bad cardinality estimate` reports immediately
