# Exercise 002: Query Optimizer Lab State and Invariants

Shared concept chapter: [write-ahead-logs.md](../../../../../curriculum/concepts/storage/write-ahead-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Query Optimizer Lab**. The work is centered on `apply_query_optimizer_lab_query_predicate_event`, not on a generic scaffold. You will implement behavior for **query predicate** moving through **plan space** while preserving the project invariant.

The implementation target is: Implement `apply_query_optimizer_lab_query_predicate_event` so Query Optimizer Lab can replay query predicate events into plan space without duplicating or accepting stale state.
...
plan space must advance monotonically by version, and duplicate query predicate events must be idempotent.
...
1. What state does `apply_query_optimizer_lab_query_predicate_event` own?
...
Implement `apply_query_optimizer_lab_query_predicate_event` so Query Optimizer Lab can replay query predicate events into plan space without duplicating or accepting stale state.
