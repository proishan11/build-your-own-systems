# Exercise 005: MiniDB Storage Engine Integration Scenario

Shared concept chapter: [write-ahead-logs.md](../../../../../curriculum/concepts/storage/write-ahead-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **MiniDB Storage Engine**. The work is centered on `run_minidb_storage_engine_scenario`, not on a generic scaffold. You will implement behavior for **database record** moving through **page cache** while preserving the project invariant.

The implementation target is: Implement `run_minidb_storage_engine_scenario` so MiniDB Storage Engine has a deterministic integration simulation with state, `pages_flushed`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose page cache state and pages_flushed metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `page cache=ready`, recording `pages_flushed=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_minidb_storage_engine_scenario` own?
...
Implement `run_minidb_storage_engine_scenario` so MiniDB Storage Engine has a deterministic integration simulation with state, `pages_flushed`, failures, recoveries, and invariant reporting.
...
- track `pages_flushed`, failures, and recoveries
