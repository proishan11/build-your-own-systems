# Exercise 004: MiniDB Storage Engine Failure and Recovery

Shared concept chapter: [write-ahead-logs.md](../../../../../curriculum/concepts/storage/write-ahead-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **MiniDB Storage Engine**. The work is centered on `recover_minidb_storage_engine_dirty_page_loss`, not on a generic scaffold. You will implement behavior for **database record** moving through **page cache** while preserving the project invariant.

The implementation target is: Implement `recover_minidb_storage_engine_dirty_page_loss` so MiniDB Storage Engine handles transient failures, permanent `dirty page loss` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for page cache must never turn a permanent dirty page loss into an unsafe retry.
...
1. What state does `recover_minidb_storage_engine_dirty_page_loss` own?
...
Implement `recover_minidb_storage_engine_dirty_page_loss` so MiniDB Storage Engine handles transient failures, permanent `dirty page loss` cases, and exhausted retry budgets explicitly.
...
- fail permanent `dirty page loss` reports immediately
