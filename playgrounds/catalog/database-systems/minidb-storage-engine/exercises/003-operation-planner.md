# Exercise 003: MiniDB Storage Engine Planning and Ordering

Shared concept chapter: [write-ahead-logs.md](../../../../../curriculum/concepts/storage/write-ahead-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **MiniDB Storage Engine**. The work is centered on `plan_minidb_storage_engine_write_records`, not on a generic scaffold. You will implement behavior for **database record** moving through **page cache** while preserving the project invariant.

The implementation target is: Implement `plan_minidb_storage_engine_write_records` so MiniDB Storage Engine can produce a deterministic write record plan from desired and observed page cache state.
...
The same desired and observed page cache states must always produce the same minimal ordered write record plan.

## Tiny Example

If `page-cache-primary` is stale, `page-cache-old` is extra, and `page-cache-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_minidb_storage_engine_write_records` own?
...
Implement `plan_minidb_storage_engine_write_records` so MiniDB Storage Engine can produce a deterministic write record plan from desired and observed page cache state.
