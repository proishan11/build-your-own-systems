# Exercise 001: MiniDB Storage Engine First Implementation Lab

Shared concept chapter: [write-ahead-logs.md](../../../../../curriculum/concepts/storage/write-ahead-logs.md)

## Concept Primer

This first lab turns **MiniDB Storage Engine** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Implement redo recovery over key/value WAL records.
...
A database component turns logical operations into durable, inspectable state. Every byte layout or index decision should help later reads, recovery, or consistency checks.
...
After each public operation, the component must preserve this contract: apply records in LSN order; ignore duplicate LSNs during replay; support put and delete records. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
- In-memory correctness automatically becomes durable correctness.
- Index order is a convenience rather than a contract.
- Recovery can infer intent from incomplete state.
...
Implement redo recovery over key/value WAL records.
...
- ignore duplicate LSNs during replay
- support put and delete records
- return recovered key/value state

## Design Hints

- Make the on-disk or logical representation explicit first.
- Keep ordering and identity stable across operations.
- Add the edge case where state is empty, full, or partially updated.
...
- Write-ahead logs: ../../../../../curriculum/concepts/storage/write-ahead-logs.md
