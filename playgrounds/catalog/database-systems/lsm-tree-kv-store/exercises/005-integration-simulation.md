# Exercise 005: LSM Tree KV Store Integration Scenario

Shared concept chapter: [write-ahead-logs.md](../../../../../curriculum/concepts/storage/write-ahead-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **LSM Tree KV Store**. The work is centered on `run_lsm_tree_kv_store_scenario`, not on a generic scaffold. You will implement behavior for **memtable write** moving through **sstable level** while preserving the project invariant.

The implementation target is: Implement `run_lsm_tree_kv_store_scenario` so LSM Tree KV Store has a deterministic integration simulation with state, `compactions_run`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose sstable level state and compactions_run metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `sstable level=ready`, recording `compactions_run=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_lsm_tree_kv_store_scenario` own?
...
Implement `run_lsm_tree_kv_store_scenario` so LSM Tree KV Store has a deterministic integration simulation with state, `compactions_run`, failures, recoveries, and invariant reporting.
...
- track `compactions_run`, failures, and recoveries
