# Exercise 002: LSM Tree KV Store State and Invariants

Shared concept chapter: [write-ahead-logs.md](../../../../../curriculum/concepts/storage/write-ahead-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **LSM Tree KV Store**. The work is centered on `apply_lsm_tree_kv_store_memtable_write_event`, not on a generic scaffold. You will implement behavior for **memtable write** moving through **sstable level** while preserving the project invariant.

The implementation target is: Implement `apply_lsm_tree_kv_store_memtable_write_event` so LSM Tree KV Store can replay memtable write events into sstable level without duplicating or accepting stale state.
...
sstable level must advance monotonically by version, and duplicate memtable write events must be idempotent.
...
1. What state does `apply_lsm_tree_kv_store_memtable_write_event` own?
...
Implement `apply_lsm_tree_kv_store_memtable_write_event` so LSM Tree KV Store can replay memtable write events into sstable level without duplicating or accepting stale state.
