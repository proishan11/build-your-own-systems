# Exercise 001: LSM Tree KV Store First Implementation Lab

Shared concept chapter: [write-ahead-logs.md](../../../../../curriculum/concepts/storage/write-ahead-logs.md)

## Concept Primer

This first lab turns **LSM Tree KV Store** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Implement sorted-string-table lookup with tombstones.
...
After each public operation, the component must preserve this contract: store sorted key/value entries; find keys by binary search; treat tombstones as deleted. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Implement sorted-string-table lookup with tombstones.
...
- find keys by binary search
- treat tombstones as deleted
- prefer newer table values during merge lookup
