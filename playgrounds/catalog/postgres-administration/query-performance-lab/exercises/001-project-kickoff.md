# Exercise 001: Query Performance Lab First Implementation Lab

Shared concept chapter: [postgres-admin-mental-model.md](../../../../../curriculum/concepts/postgres/postgres-admin-mental-model.md)

## Concept Primer

This first lab turns **Query Performance Lab** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Analyze simple query stats and recommend an index candidate.
...
After each public operation, the component must preserve this contract: rank queries by total time; identify filter columns; avoid recommending indexes for low-frequency queries. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Analyze simple query stats and recommend an index candidate.
...
- rank queries by total time
- identify filter columns
- avoid recommending indexes for low-frequency queries
