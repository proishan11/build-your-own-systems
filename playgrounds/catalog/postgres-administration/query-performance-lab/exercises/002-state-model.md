# Exercise 002: Query Performance Lab State and Invariants

Shared concept chapter: [postgres-admin-mental-model.md](../../../../../curriculum/concepts/postgres/postgres-admin-mental-model.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Query Performance Lab**. The work is centered on `apply_query_performance_lab_query_plan_event`, not on a generic scaffold. You will implement behavior for **query plan** moving through **index catalog** while preserving the project invariant.

The implementation target is: Implement `apply_query_performance_lab_query_plan_event` so Query Performance Lab can replay query plan events into index catalog without duplicating or accepting stale state.
...
index catalog must advance monotonically by version, and duplicate query plan events must be idempotent.
...
1. What state does `apply_query_performance_lab_query_plan_event` own?
...
Implement `apply_query_performance_lab_query_plan_event` so Query Performance Lab can replay query plan events into index catalog without duplicating or accepting stale state.
