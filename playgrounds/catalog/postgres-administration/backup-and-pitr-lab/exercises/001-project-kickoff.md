# Exercise 001: Backup and PITR Lab First Implementation Lab

Shared concept chapter: [postgres-admin-mental-model.md](../../../../../curriculum/concepts/postgres/postgres-admin-mental-model.md)

## Concept Primer

This first lab turns **Backup and PITR Lab** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Choose a base backup and WAL segment set for point-in-time recovery.
...
After each public operation, the component must preserve this contract: select latest backup before target time; include WAL after backup through target; reject targets before any backup. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Choose a base backup and WAL segment set for point-in-time recovery.
...
- select latest backup before target time
- include WAL after backup through target
- reject targets before any backup
- return restore plan
