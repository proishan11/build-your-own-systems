# Exercise 001: Backup and PITR Lab Core Mechanism

Shared concept chapter: [postgres-admin-mental-model.md](../../../../../curriculum/concepts/postgres/postgres-admin-mental-model.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Backup and PITR Lab**. The work is centered on `restore_target_wal_segment`, not on a generic scaffold. You will implement behavior for **wal segment** moving through **backup catalog** while preserving the project invariant.

The implementation target is: Implement `restore_target_wal_segment` so Backup and PITR Lab has a concrete implementation boundary for wal segment requests before they touch backup catalog.
...
Only valid wal segment requests may become restore target operations against backup catalog; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `wal-segment-001`, kind `restore target`, and target `backup catalog` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `restore_target_wal_segment` own?
...
Implement `restore_target_wal_segment` so Backup and PITR Lab has a concrete implementation boundary for wal segment requests before they touch backup catalog.
...
- build valid wal segment requests into a stable project operation
...
- Treat `missing base backup` as the kind of bad input that must never reach mutation code.
