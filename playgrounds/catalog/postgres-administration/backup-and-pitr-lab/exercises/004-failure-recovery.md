# Exercise 004: Backup and PITR Lab Failure and Recovery

Shared concept chapter: [postgres-admin-mental-model.md](../../../../../curriculum/concepts/postgres/postgres-admin-mental-model.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Backup and PITR Lab**. The work is centered on `recover_backup_and_pitr_lab_missing_base_backup`, not on a generic scaffold. You will implement behavior for **wal segment** moving through **backup catalog** while preserving the project invariant.

The implementation target is: Implement `recover_backup_and_pitr_lab_missing_base_backup` so Backup and PITR Lab handles transient failures, permanent `missing base backup` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for backup catalog must never turn a permanent missing base backup into an unsafe retry.
...
1. What state does `recover_backup_and_pitr_lab_missing_base_backup` own?
...
Implement `recover_backup_and_pitr_lab_missing_base_backup` so Backup and PITR Lab handles transient failures, permanent `missing base backup` cases, and exhausted retry budgets explicitly.
...
- fail permanent `missing base backup` reports immediately
