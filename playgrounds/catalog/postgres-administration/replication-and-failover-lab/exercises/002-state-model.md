# Exercise 002: Replication and Failover Lab State and Invariants

Shared concept chapter: [postgres-admin-mental-model.md](../../../../../curriculum/concepts/postgres/postgres-admin-mental-model.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Replication and Failover Lab**. The work is centered on `apply_replication_and_failover_lab_replica_status_event`, not on a generic scaffold. You will implement behavior for **replica status** moving through **failover plan** while preserving the project invariant.

The implementation target is: Implement `apply_replication_and_failover_lab_replica_status_event` so Replication and Failover Lab can replay replica status events into failover plan without duplicating or accepting stale state.
...
failover plan must advance monotonically by version, and duplicate replica status events must be idempotent.
...
1. What state does `apply_replication_and_failover_lab_replica_status_event` own?
...
Implement `apply_replication_and_failover_lab_replica_status_event` so Replication and Failover Lab can replay replica status events into failover plan without duplicating or accepting stale state.
