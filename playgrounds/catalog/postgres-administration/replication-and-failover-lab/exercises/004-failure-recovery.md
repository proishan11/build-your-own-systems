# Exercise 004: Replication and Failover Lab Failure and Recovery

Shared concept chapter: [postgres-admin-mental-model.md](../../../../../curriculum/concepts/postgres/postgres-admin-mental-model.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Replication and Failover Lab**. The work is centered on `recover_replication_and_failover_lab_split_brain`, not on a generic scaffold. You will implement behavior for **replica status** moving through **failover plan** while preserving the project invariant.

The implementation target is: Implement `recover_replication_and_failover_lab_split_brain` so Replication and Failover Lab handles transient failures, permanent `split brain` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for failover plan must never turn a permanent split brain into an unsafe retry.
...
1. What state does `recover_replication_and_failover_lab_split_brain` own?
...
Implement `recover_replication_and_failover_lab_split_brain` so Replication and Failover Lab handles transient failures, permanent `split brain` cases, and exhausted retry budgets explicitly.
...
- fail permanent `split brain` reports immediately
