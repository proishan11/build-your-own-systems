# Exercise 003: Shell Text Processing Workbench Planning and Ordering

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Shell Text Processing Workbench**. The work is centered on `plan_shell_text_processing_workbench_filter_records`, not on a generic scaffold. You will implement behavior for **text record** moving through **pipeline stage** while preserving the project invariant.

The implementation target is: Implement `plan_shell_text_processing_workbench_filter_records` so Shell Text Processing Workbench can produce a deterministic filter record plan from desired and observed pipeline stage state.
...
The same desired and observed pipeline stage states must always produce the same minimal ordered filter record plan.

## Tiny Example

If `pipeline-stage-primary` is stale, `pipeline-stage-old` is extra, and `pipeline-stage-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_shell_text_processing_workbench_filter_records` own?
...
Implement `plan_shell_text_processing_workbench_filter_records` so Shell Text Processing Workbench can produce a deterministic filter record plan from desired and observed pipeline stage state.
