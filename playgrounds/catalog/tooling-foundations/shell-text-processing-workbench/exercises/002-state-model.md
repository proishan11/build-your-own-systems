# Exercise 002: Shell Text Processing Workbench State and Invariants

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Shell Text Processing Workbench**. The work is centered on `apply_shell_text_processing_workbench_text_record_event`, not on a generic scaffold. You will implement behavior for **text record** moving through **pipeline stage** while preserving the project invariant.

The implementation target is: Implement `apply_shell_text_processing_workbench_text_record_event` so Shell Text Processing Workbench can replay text record events into pipeline stage without duplicating or accepting stale state.
...
pipeline stage must advance monotonically by version, and duplicate text record events must be idempotent.
...
1. What state does `apply_shell_text_processing_workbench_text_record_event` own?
...
Implement `apply_shell_text_processing_workbench_text_record_event` so Shell Text Processing Workbench can replay text record events into pipeline stage without duplicating or accepting stale state.
