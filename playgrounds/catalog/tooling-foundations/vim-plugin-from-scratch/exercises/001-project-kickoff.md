# Exercise 001: Vim Plugin From Scratch Core Mechanism

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Vim Plugin From Scratch**. The work is centered on `handle_autocmd_plugin_event`, not on a generic scaffold. You will implement behavior for **plugin event** moving through **editor state** while preserving the project invariant.

The implementation target is: Implement `handle_autocmd_plugin_event` so Vim Plugin From Scratch has a concrete implementation boundary for plugin event requests before they touch editor state.
...
Only valid plugin event requests may become handle autocmd operations against editor state; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `plugin-event-001`, kind `handle autocmd`, and target `editor state` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `handle_autocmd_plugin_event` own?
...
Implement `handle_autocmd_plugin_event` so Vim Plugin From Scratch has a concrete implementation boundary for plugin event requests before they touch editor state.
...
- build valid plugin event requests into a stable project operation
...
- Treat `recursive mapping` as the kind of bad input that must never reach mutation code.
