# Exercise 002: Git Merge and Rebase Lab State and Invariants

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Git Merge and Rebase Lab**. The work is centered on `apply_git_merge_and_rebase_lab_commit_change_event`, not on a generic scaffold. You will implement behavior for **commit change** moving through **merge graph** while preserving the project invariant.

The implementation target is: Implement `apply_git_merge_and_rebase_lab_commit_change_event` so Git Merge and Rebase Lab can replay commit change events into merge graph without duplicating or accepting stale state.
...
merge graph must advance monotonically by version, and duplicate commit change events must be idempotent.
...
1. What state does `apply_git_merge_and_rebase_lab_commit_change_event` own?
...
Implement `apply_git_merge_and_rebase_lab_commit_change_event` so Git Merge and Rebase Lab can replay commit change events into merge graph without duplicating or accepting stale state.
