# Exercise 002: Coreutils From Scratch State and Invariants

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Coreutils From Scratch**. The work is centered on `apply_coreutils_from_scratch_file_stream_event`, not on a generic scaffold. You will implement behavior for **file stream** moving through **command input** while preserving the project invariant.

The implementation target is: Implement `apply_coreutils_from_scratch_file_stream_event` so Coreutils From Scratch can replay file stream events into command input without duplicating or accepting stale state.
...
command input must advance monotonically by version, and duplicate file stream events must be idempotent.
...
1. What state does `apply_coreutils_from_scratch_file_stream_event` own?
...
Implement `apply_coreutils_from_scratch_file_stream_event` so Coreutils From Scratch can replay file stream events into command input without duplicating or accepting stale state.
