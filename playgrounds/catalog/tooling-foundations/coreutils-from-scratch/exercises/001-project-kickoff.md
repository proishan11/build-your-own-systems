# Exercise 001: Coreutils From Scratch Core Mechanism

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Coreutils From Scratch**. The work is centered on `transform_stream_file_stream`, not on a generic scaffold. You will implement behavior for **file stream** moving through **command input** while preserving the project invariant.

The implementation target is: Implement `transform_stream_file_stream` so Coreutils From Scratch has a concrete implementation boundary for file stream requests before they touch command input.
...
Only valid file stream requests may become transform stream operations against command input; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `file-stream-001`, kind `transform stream`, and target `command input` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `transform_stream_file_stream` own?
...
Implement `transform_stream_file_stream` so Coreutils From Scratch has a concrete implementation boundary for file stream requests before they touch command input.
...
- build valid file stream requests into a stable project operation
...
- Treat `binary input` as the kind of bad input that must never reach mutation code.
