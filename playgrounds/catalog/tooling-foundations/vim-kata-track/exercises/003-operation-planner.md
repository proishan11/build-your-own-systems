# Exercise 003: Vim Kata Track Planning and Ordering

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Vim Kata Track**. The work is centered on `plan_vim_kata_track_apply_motions`, not on a generic scaffold. You will implement behavior for **editor command** moving through **buffer state** while preserving the project invariant.

The implementation target is: Implement `plan_vim_kata_track_apply_motions` so Vim Kata Track can produce a deterministic apply motion plan from desired and observed buffer state state.
...
The same desired and observed buffer state states must always produce the same minimal ordered apply motion plan.

## Tiny Example

If `buffer-state-primary` is stale, `buffer-state-old` is extra, and `buffer-state-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_vim_kata_track_apply_motions` own?
...
Implement `plan_vim_kata_track_apply_motions` so Vim Kata Track can produce a deterministic apply motion plan from desired and observed buffer state state.
