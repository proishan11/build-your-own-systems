# Exercise 002: Vim Kata Track State and Invariants

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Vim Kata Track**. The work is centered on `apply_vim_kata_track_editor_command_event`, not on a generic scaffold. You will implement behavior for **editor command** moving through **buffer state** while preserving the project invariant.

The implementation target is: Implement `apply_vim_kata_track_editor_command_event` so Vim Kata Track can replay editor command events into buffer state without duplicating or accepting stale state.
...
buffer state must advance monotonically by version, and duplicate editor command events must be idempotent.
...
1. What state does `apply_vim_kata_track_editor_command_event` own?
...
Implement `apply_vim_kata_track_editor_command_event` so Vim Kata Track can replay editor command events into buffer state without duplicating or accepting stale state.
