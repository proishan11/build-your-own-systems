# Exercise 005: Vim Kata Track Integration Scenario

Shared concept chapter: [unix-pipelines.md](../../../../../curriculum/concepts/developer-tools/unix-pipelines.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Vim Kata Track**. The work is centered on `run_vim_kata_track_scenario`, not on a generic scaffold. You will implement behavior for **editor command** moving through **buffer state** while preserving the project invariant.

The implementation target is: Implement `run_vim_kata_track_scenario` so Vim Kata Track has a deterministic integration simulation with state, `motions_applied`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose buffer state state and motions_applied metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `buffer state=ready`, recording `motions_applied=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_vim_kata_track_scenario` own?
...
Implement `run_vim_kata_track_scenario` so Vim Kata Track has a deterministic integration simulation with state, `motions_applied`, failures, recoveries, and invariant reporting.
...
- track `motions_applied`, failures, and recoveries
