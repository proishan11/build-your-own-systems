# Exercise 004: Model Router Failure and Recovery
...
This exercise is a project-specific implementation milestone for **Model Router**. The work is centered on `recover_model_router_context_overflow`, not on a generic scaffold. You will implement behavior for **llm request** moving through **model catalog** while preserving the project invariant.

The implementation target is: Implement `recover_model_router_context_overflow` so Model Router handles transient failures, permanent `context overflow` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for model catalog must never turn a permanent context overflow into an unsafe retry.
...
1. What state does `recover_model_router_context_overflow` own?
...
Implement `recover_model_router_context_overflow` so Model Router handles transient failures, permanent `context overflow` cases, and exhausted retry budgets explicitly.
...
- fail permanent `context overflow` reports immediately
