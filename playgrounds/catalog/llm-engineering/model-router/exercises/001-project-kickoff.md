# Exercise 001: Model Router First Implementation Lab
...
This first lab turns **Model Router** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Implement request routing across models using context length, cost, and latency constraints.
...
After each public operation, the component must preserve this contract: register model capabilities; filter by context length; filter by max cost. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Implement request routing across models using context length, cost, and latency constraints.
...
- filter by max cost
- choose the lowest-latency eligible model
