# Exercise 002: Model Router State and Invariants
...
This exercise is a project-specific implementation milestone for **Model Router**. The work is centered on `apply_model_router_llm_request_event`, not on a generic scaffold. You will implement behavior for **llm request** moving through **model catalog** while preserving the project invariant.

The implementation target is: Implement `apply_model_router_llm_request_event` so Model Router can replay llm request events into model catalog without duplicating or accepting stale state.
...
model catalog must advance monotonically by version, and duplicate llm request events must be idempotent.
...
1. What state does `apply_model_router_llm_request_event` own?
...
Implement `apply_model_router_llm_request_event` so Model Router can replay llm request events into model catalog without duplicating or accepting stale state.
