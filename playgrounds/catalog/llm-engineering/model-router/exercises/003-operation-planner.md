# Exercise 003: Model Router Planning and Ordering
...
This exercise is a project-specific implementation milestone for **Model Router**. The work is centered on `plan_model_router_route_models`, not on a generic scaffold. You will implement behavior for **llm request** moving through **model catalog** while preserving the project invariant.

The implementation target is: Implement `plan_model_router_route_models` so Model Router can produce a deterministic route model plan from desired and observed model catalog state.
...
The same desired and observed model catalog states must always produce the same minimal ordered route model plan.

## Tiny Example

If `model-catalog-primary` is stale, `model-catalog-old` is extra, and `model-catalog-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_model_router_route_models` own?
...
Implement `plan_model_router_route_models` so Model Router can produce a deterministic route model plan from desired and observed model catalog state.
