# Exercise 001: Cost and Latency Dashboard Core Mechanism
...
This exercise is a project-specific implementation milestone for **Cost and Latency Dashboard**. The work is centered on `aggregate_sample_usage_sample`, not on a generic scaffold. You will implement behavior for **usage sample** moving through **metric bucket** while preserving the project invariant.

The implementation target is: Implement `aggregate_sample_usage_sample` so Cost and Latency Dashboard has a concrete implementation boundary for usage sample requests before they touch metric bucket.
...
Only valid usage sample requests may become aggregate sample operations against metric bucket; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `usage-sample-001`, kind `aggregate sample`, and target `metric bucket` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `aggregate_sample_usage_sample` own?
...
Implement `aggregate_sample_usage_sample` so Cost and Latency Dashboard has a concrete implementation boundary for usage sample requests before they touch metric bucket.
...
- build valid usage sample requests into a stable project operation
...
- Treat `tail spike` as the kind of bad input that must never reach mutation code.
