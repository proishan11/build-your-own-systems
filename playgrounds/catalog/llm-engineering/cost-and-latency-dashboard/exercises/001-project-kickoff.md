# Exercise 001: Cost and Latency Dashboard First Implementation Lab
...
This first lab turns **Cost and Latency Dashboard** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Aggregate LLM usage into cost and latency metrics.
...
After each public operation, the component must preserve this contract: sum cost by route; compute p95 latency; count requests. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Aggregate LLM usage into cost and latency metrics.
...
- compute p95 latency
- count requests
- separate model latency from tool latency
