# Exercise 004: Cost and Latency Dashboard Failure and Recovery
...
This exercise is a project-specific implementation milestone for **Cost and Latency Dashboard**. The work is centered on `recover_cost_and_latency_dashboard_tail_spike`, not on a generic scaffold. You will implement behavior for **usage sample** moving through **metric bucket** while preserving the project invariant.

The implementation target is: Implement `recover_cost_and_latency_dashboard_tail_spike` so Cost and Latency Dashboard handles transient failures, permanent `tail spike` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for metric bucket must never turn a permanent tail spike into an unsafe retry.
...
1. What state does `recover_cost_and_latency_dashboard_tail_spike` own?
...
Implement `recover_cost_and_latency_dashboard_tail_spike` so Cost and Latency Dashboard handles transient failures, permanent `tail spike` cases, and exhausted retry budgets explicitly.
...
- fail permanent `tail spike` reports immediately
