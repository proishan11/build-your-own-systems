# Exercise 005: Cost and Latency Dashboard Integration Scenario
...
This exercise is a project-specific implementation milestone for **Cost and Latency Dashboard**. The work is centered on `run_cost_and_latency_dashboard_scenario`, not on a generic scaffold. You will implement behavior for **usage sample** moving through **metric bucket** while preserving the project invariant.

The implementation target is: Implement `run_cost_and_latency_dashboard_scenario` so Cost and Latency Dashboard has a deterministic integration simulation with state, `p95_latency_ms`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose metric bucket state and p95_latency_ms metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `metric bucket=ready`, recording `p95_latency_ms=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_cost_and_latency_dashboard_scenario` own?
...
Implement `run_cost_and_latency_dashboard_scenario` so Cost and Latency Dashboard has a deterministic integration simulation with state, `p95_latency_ms`, failures, recoveries, and invariant reporting.
...
- track `p95_latency_ms`, failures, and recoveries
