# Exercise 004: Sandboxed Coding Agent Failure and Recovery
...
This exercise is a project-specific implementation milestone for **Sandboxed Coding Agent**. The work is centered on `recover_sandboxed_coding_agent_filesystem_escape`, not on a generic scaffold. You will implement behavior for **tool invocation** moving through **sandbox policy** while preserving the project invariant.

The implementation target is: Implement `recover_sandboxed_coding_agent_filesystem_escape` so Sandboxed Coding Agent handles transient failures, permanent `filesystem escape` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for sandbox policy must never turn a permanent filesystem escape into an unsafe retry.
...
1. What state does `recover_sandboxed_coding_agent_filesystem_escape` own?
...
Implement `recover_sandboxed_coding_agent_filesystem_escape` so Sandboxed Coding Agent handles transient failures, permanent `filesystem escape` cases, and exhausted retry budgets explicitly.
...
- fail permanent `filesystem escape` reports immediately
