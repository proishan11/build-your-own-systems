# Exercise 001: Sandboxed Coding Agent First Implementation Lab
...
This first lab turns **Sandboxed Coding Agent** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Implement a command policy for a sandboxed agent.
...
After each public operation, the component must preserve this contract: allow read-only commands; reject destructive commands; classify commands that require approval. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Implement a command policy for a sandboxed agent.
...
- classify commands that require approval
