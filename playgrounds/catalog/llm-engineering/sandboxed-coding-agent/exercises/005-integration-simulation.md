# Exercise 005: Sandboxed Coding Agent Integration Scenario
...
This exercise is a project-specific implementation milestone for **Sandboxed Coding Agent**. The work is centered on `run_sandboxed_coding_agent_scenario`, not on a generic scaffold. You will implement behavior for **tool invocation** moving through **sandbox policy** while preserving the project invariant.

The implementation target is: Implement `run_sandboxed_coding_agent_scenario` so Sandboxed Coding Agent has a deterministic integration simulation with state, `tool_calls_allowed`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose sandbox policy state and tool_calls_allowed metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `sandbox policy=ready`, recording `tool_calls_allowed=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_sandboxed_coding_agent_scenario` own?
...
Implement `run_sandboxed_coding_agent_scenario` so Sandboxed Coding Agent has a deterministic integration simulation with state, `tool_calls_allowed`, failures, recoveries, and invariant reporting.
...
- track `tool_calls_allowed`, failures, and recoveries
