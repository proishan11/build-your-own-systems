# Exercise 003: Sandboxed Coding Agent Planning and Ordering
...
This exercise is a project-specific implementation milestone for **Sandboxed Coding Agent**. The work is centered on `plan_sandboxed_coding_agent_authorize_tools`, not on a generic scaffold. You will implement behavior for **tool invocation** moving through **sandbox policy** while preserving the project invariant.

The implementation target is: Implement `plan_sandboxed_coding_agent_authorize_tools` so Sandboxed Coding Agent can produce a deterministic authorize tool plan from desired and observed sandbox policy state.
...
The same desired and observed sandbox policy states must always produce the same minimal ordered authorize tool plan.

## Tiny Example

If `sandbox-policy-primary` is stale, `sandbox-policy-old` is extra, and `sandbox-policy-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_sandboxed_coding_agent_authorize_tools` own?
...
Implement `plan_sandboxed_coding_agent_authorize_tools` so Sandboxed Coding Agent can produce a deterministic authorize tool plan from desired and observed sandbox policy state.
