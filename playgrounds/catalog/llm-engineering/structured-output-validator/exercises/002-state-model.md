# Exercise 002: Structured Output Validator State and Invariants
...
This exercise is a project-specific implementation milestone for **Structured Output Validator**. The work is centered on `apply_structured_output_validator_model_output_event`, not on a generic scaffold. You will implement behavior for **model output** moving through **json schema** while preserving the project invariant.

The implementation target is: Implement `apply_structured_output_validator_model_output_event` so Structured Output Validator can replay model output events into json schema without duplicating or accepting stale state.
...
json schema must advance monotonically by version, and duplicate model output events must be idempotent.
...
1. What state does `apply_structured_output_validator_model_output_event` own?
...
Implement `apply_structured_output_validator_model_output_event` so Structured Output Validator can replay model output events into json schema without duplicating or accepting stale state.
