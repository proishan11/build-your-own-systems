# Exercise 004: Structured Output Validator Failure and Recovery
...
This exercise is a project-specific implementation milestone for **Structured Output Validator**. The work is centered on `recover_structured_output_validator_schema_violation`, not on a generic scaffold. You will implement behavior for **model output** moving through **json schema** while preserving the project invariant.

The implementation target is: Implement `recover_structured_output_validator_schema_violation` so Structured Output Validator handles transient failures, permanent `schema violation` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for json schema must never turn a permanent schema violation into an unsafe retry.
...
1. What state does `recover_structured_output_validator_schema_violation` own?
...
Implement `recover_structured_output_validator_schema_violation` so Structured Output Validator handles transient failures, permanent `schema violation` cases, and exhausted retry budgets explicitly.
...
- fail permanent `schema violation` reports immediately
