# Exercise 003: Structured Output Validator Planning and Ordering
...
This exercise is a project-specific implementation milestone for **Structured Output Validator**. The work is centered on `plan_structured_output_validator_validate_outputs`, not on a generic scaffold. You will implement behavior for **model output** moving through **json schema** while preserving the project invariant.

The implementation target is: Implement `plan_structured_output_validator_validate_outputs` so Structured Output Validator can produce a deterministic validate output plan from desired and observed json schema state.
...
The same desired and observed json schema states must always produce the same minimal ordered validate output plan.

## Tiny Example

If `json-schema-primary` is stale, `json-schema-old` is extra, and `json-schema-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_structured_output_validator_validate_outputs` own?
...
Implement `plan_structured_output_validator_validate_outputs` so Structured Output Validator can produce a deterministic validate output plan from desired and observed json schema state.
