# Exercise 005: Structured Output Validator Integration Scenario
...
This exercise is a project-specific implementation milestone for **Structured Output Validator**. The work is centered on `run_structured_output_validator_scenario`, not on a generic scaffold. You will implement behavior for **model output** moving through **json schema** while preserving the project invariant.

The implementation target is: Implement `run_structured_output_validator_scenario` so Structured Output Validator has a deterministic integration simulation with state, `outputs_validated`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose json schema state and outputs_validated metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `json schema=ready`, recording `outputs_validated=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_structured_output_validator_scenario` own?
...
Implement `run_structured_output_validator_scenario` so Structured Output Validator has a deterministic integration simulation with state, `outputs_validated`, failures, recoveries, and invariant reporting.
...
- track `outputs_validated`, failures, and recoveries
