# Exercise 004: LLM Trace Store Failure and Recovery
...
This exercise is a project-specific implementation milestone for **LLM Trace Store**. The work is centered on `recover_llm_trace_store_missing_parent_span`, not on a generic scaffold. You will implement behavior for **llm span** moving through **trace index** while preserving the project invariant.

The implementation target is: Implement `recover_llm_trace_store_missing_parent_span` so LLM Trace Store handles transient failures, permanent `missing parent span` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for trace index must never turn a permanent missing parent span into an unsafe retry.
...
1. What state does `recover_llm_trace_store_missing_parent_span` own?
...
Implement `recover_llm_trace_store_missing_parent_span` so LLM Trace Store handles transient failures, permanent `missing parent span` cases, and exhausted retry budgets explicitly.
...
- fail permanent `missing parent span` reports immediately
