# Exercise 003: LLM Trace Store Planning and Ordering
...
This exercise is a project-specific implementation milestone for **LLM Trace Store**. The work is centered on `plan_llm_trace_store_record_spans`, not on a generic scaffold. You will implement behavior for **llm span** moving through **trace index** while preserving the project invariant.

The implementation target is: Implement `plan_llm_trace_store_record_spans` so LLM Trace Store can produce a deterministic record span plan from desired and observed trace index state.
...
The same desired and observed trace index states must always produce the same minimal ordered record span plan.

## Tiny Example

If `trace-index-primary` is stale, `trace-index-old` is extra, and `trace-index-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_llm_trace_store_record_spans` own?
...
Implement `plan_llm_trace_store_record_spans` so LLM Trace Store can produce a deterministic record span plan from desired and observed trace index state.
