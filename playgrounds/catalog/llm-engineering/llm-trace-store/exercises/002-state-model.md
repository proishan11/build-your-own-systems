# Exercise 002: LLM Trace Store State and Invariants
...
This exercise is a project-specific implementation milestone for **LLM Trace Store**. The work is centered on `apply_llm_trace_store_llm_span_event`, not on a generic scaffold. You will implement behavior for **llm span** moving through **trace index** while preserving the project invariant.

The implementation target is: Implement `apply_llm_trace_store_llm_span_event` so LLM Trace Store can replay llm span events into trace index without duplicating or accepting stale state.
...
trace index must advance monotonically by version, and duplicate llm span events must be idempotent.
...
1. What state does `apply_llm_trace_store_llm_span_event` own?
...
Implement `apply_llm_trace_store_llm_span_event` so LLM Trace Store can replay llm span events into trace index without duplicating or accepting stale state.
