# Exercise 001: LLM Trace Store Core Mechanism
...
This exercise is a project-specific implementation milestone for **LLM Trace Store**. The work is centered on `record_span_llm_span`, not on a generic scaffold. You will implement behavior for **llm span** moving through **trace index** while preserving the project invariant.

The implementation target is: Implement `record_span_llm_span` so LLM Trace Store has a concrete implementation boundary for llm span requests before they touch trace index.
...
Only valid llm span requests may become record span operations against trace index; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `llm-span-001`, kind `record span`, and target `trace index` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `record_span_llm_span` own?
...
Implement `record_span_llm_span` so LLM Trace Store has a concrete implementation boundary for llm span requests before they touch trace index.
...
- build valid llm span requests into a stable project operation
...
- Treat `missing parent span` as the kind of bad input that must never reach mutation code.
