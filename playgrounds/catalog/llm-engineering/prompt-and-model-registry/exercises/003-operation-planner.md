# Exercise 003: Prompt and Model Registry Planning and Ordering
...
This exercise is a project-specific implementation milestone for **Prompt and Model Registry**. The work is centered on `plan_prompt_and_model_registry_publish_prompts`, not on a generic scaffold. You will implement behavior for **prompt template** moving through **model binding** while preserving the project invariant.

The implementation target is: Implement `plan_prompt_and_model_registry_publish_prompts` so Prompt and Model Registry can produce a deterministic publish prompt plan from desired and observed model binding state.
...
The same desired and observed model binding states must always produce the same minimal ordered publish prompt plan.

## Tiny Example

If `model-binding-primary` is stale, `model-binding-old` is extra, and `model-binding-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_prompt_and_model_registry_publish_prompts` own?
...
Implement `plan_prompt_and_model_registry_publish_prompts` so Prompt and Model Registry can produce a deterministic publish prompt plan from desired and observed model binding state.
