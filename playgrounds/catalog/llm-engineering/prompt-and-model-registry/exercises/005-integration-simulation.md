# Exercise 005: Prompt and Model Registry Integration Scenario
...
This exercise is a project-specific implementation milestone for **Prompt and Model Registry**. The work is centered on `run_prompt_and_model_registry_scenario`, not on a generic scaffold. You will implement behavior for **prompt template** moving through **model binding** while preserving the project invariant.

The implementation target is: Implement `run_prompt_and_model_registry_scenario` so Prompt and Model Registry has a deterministic integration simulation with state, `versions_published`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose model binding state and versions_published metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `model binding=ready`, recording `versions_published=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_prompt_and_model_registry_scenario` own?
...
Implement `run_prompt_and_model_registry_scenario` so Prompt and Model Registry has a deterministic integration simulation with state, `versions_published`, failures, recoveries, and invariant reporting.
...
- track `versions_published`, failures, and recoveries
