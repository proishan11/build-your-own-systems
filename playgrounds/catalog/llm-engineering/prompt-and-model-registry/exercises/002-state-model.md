# Exercise 002: Prompt and Model Registry State and Invariants
...
This exercise is a project-specific implementation milestone for **Prompt and Model Registry**. The work is centered on `apply_prompt_and_model_registry_prompt_template_event`, not on a generic scaffold. You will implement behavior for **prompt template** moving through **model binding** while preserving the project invariant.

The implementation target is: Implement `apply_prompt_and_model_registry_prompt_template_event` so Prompt and Model Registry can replay prompt template events into model binding without duplicating or accepting stale state.
...
model binding must advance monotonically by version, and duplicate prompt template events must be idempotent.
...
1. What state does `apply_prompt_and_model_registry_prompt_template_event` own?
...
Implement `apply_prompt_and_model_registry_prompt_template_event` so Prompt and Model Registry can replay prompt template events into model binding without duplicating or accepting stale state.
