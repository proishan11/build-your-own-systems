# Exercise 001: Human Approval Workflow First Implementation Lab
...
This first lab turns **Human Approval Workflow** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Implement approval state transitions for mutating actions.
...
After each public operation, the component must preserve this contract: create pending actions; approve or reject pending actions; prevent executing unapproved actions. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Implement approval state transitions for mutating actions.
...
- prevent executing unapproved actions
