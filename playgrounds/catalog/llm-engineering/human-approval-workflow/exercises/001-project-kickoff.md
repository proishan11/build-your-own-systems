# Exercise 001: Human Approval Workflow Core Mechanism
...
This exercise is a project-specific implementation milestone for **Human Approval Workflow**. The work is centered on `route_approval_approval_request`, not on a generic scaffold. You will implement behavior for **approval request** moving through **approval queue** while preserving the project invariant.

The implementation target is: Implement `route_approval_approval_request` so Human Approval Workflow has a concrete implementation boundary for approval requests before they touch approval queue.
...
Only valid approval requests may become route approval operations against approval queue; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `approval-request-001`, kind `route approval`, and target `approval queue` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `route_approval_approval_request` own?
...
Implement `route_approval_approval_request` so Human Approval Workflow has a concrete implementation boundary for approval requests before they touch approval queue.
...
- build valid approval requests into a stable project operation
...
- Treat `stale approval` as the kind of bad input that must never reach mutation code.
