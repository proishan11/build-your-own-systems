# Exercise 005: Human Approval Workflow Integration Scenario
...
This exercise is a project-specific implementation milestone for **Human Approval Workflow**. The work is centered on `run_human_approval_workflow_scenario`, not on a generic scaffold. You will implement behavior for **approval request** moving through **approval queue** while preserving the project invariant.

The implementation target is: Implement `run_human_approval_workflow_scenario` so Human Approval Workflow has a deterministic integration simulation with state, `approvals_resolved`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose approval queue state and approvals_resolved metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `approval queue=ready`, recording `approvals_resolved=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_human_approval_workflow_scenario` own?
...
Implement `run_human_approval_workflow_scenario` so Human Approval Workflow has a deterministic integration simulation with state, `approvals_resolved`, failures, recoveries, and invariant reporting.
...
- track `approvals_resolved`, failures, and recoveries
