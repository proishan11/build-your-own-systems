# Exercise 002: Human Approval Workflow State and Invariants
...
This exercise is a project-specific implementation milestone for **Human Approval Workflow**. The work is centered on `apply_human_approval_workflow_approval_request_event`, not on a generic scaffold. You will implement behavior for **approval request** moving through **approval queue** while preserving the project invariant.

The implementation target is: Implement `apply_human_approval_workflow_approval_request_event` so Human Approval Workflow can replay approval request events into approval queue without duplicating or accepting stale state.
...
approval queue must advance monotonically by version, and duplicate approval request events must be idempotent.
...
1. What state does `apply_human_approval_workflow_approval_request_event` own?
...
Implement `apply_human_approval_workflow_approval_request_event` so Human Approval Workflow can replay approval request events into approval queue without duplicating or accepting stale state.
