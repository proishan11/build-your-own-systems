# Exercise 004: Human Approval Workflow Failure and Recovery
...
This exercise is a project-specific implementation milestone for **Human Approval Workflow**. The work is centered on `recover_human_approval_workflow_stale_approval`, not on a generic scaffold. You will implement behavior for **approval request** moving through **approval queue** while preserving the project invariant.

The implementation target is: Implement `recover_human_approval_workflow_stale_approval` so Human Approval Workflow handles transient failures, permanent `stale approval` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for approval queue must never turn a permanent stale approval into an unsafe retry.
...
1. What state does `recover_human_approval_workflow_stale_approval` own?
...
Implement `recover_human_approval_workflow_stale_approval` so Human Approval Workflow handles transient failures, permanent `stale approval` cases, and exhausted retry budgets explicitly.
...
- fail permanent `stale approval` reports immediately
