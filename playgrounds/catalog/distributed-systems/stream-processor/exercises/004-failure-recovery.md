# Exercise 004: Stream Processor Failure and Recovery

Shared concept chapter: [raft-and-replicated-logs.md](../../../../../curriculum/concepts/distributed-systems/raft-and-replicated-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Stream Processor**. The work is centered on `recover_stream_processor_late_event`, not on a generic scaffold. You will implement behavior for **stream event** moving through **watermark state** while preserving the project invariant.

The implementation target is: Implement `recover_stream_processor_late_event` so Stream Processor handles transient failures, permanent `late event` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for watermark state must never turn a permanent late event into an unsafe retry.
...
1. What state does `recover_stream_processor_late_event` own?
...
Implement `recover_stream_processor_late_event` so Stream Processor handles transient failures, permanent `late event` cases, and exhausted retry budgets explicitly.
...
- fail permanent `late event` reports immediately
