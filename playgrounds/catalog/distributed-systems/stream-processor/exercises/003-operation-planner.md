# Exercise 003: Stream Processor Planning and Ordering

Shared concept chapter: [raft-and-replicated-logs.md](../../../../../curriculum/concepts/distributed-systems/raft-and-replicated-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Stream Processor**. The work is centered on `plan_stream_processor_advance_watermarks`, not on a generic scaffold. You will implement behavior for **stream event** moving through **watermark state** while preserving the project invariant.

The implementation target is: Implement `plan_stream_processor_advance_watermarks` so Stream Processor can produce a deterministic advance watermark plan from desired and observed watermark state state.
...
The same desired and observed watermark state states must always produce the same minimal ordered advance watermark plan.

## Tiny Example

If `watermark-state-primary` is stale, `watermark-state-old` is extra, and `watermark-state-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_stream_processor_advance_watermarks` own?
...
Implement `plan_stream_processor_advance_watermarks` so Stream Processor can produce a deterministic advance watermark plan from desired and observed watermark state state.
