# Exercise 001: Stream Processor First Implementation Lab

Shared concept chapter: [raft-and-replicated-logs.md](../../../../../curriculum/concepts/distributed-systems/raft-and-replicated-logs.md)

## Concept Primer

This first lab turns **Stream Processor** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Implement tumbling-window aggregation over event timestamps.
...
After each public operation, the component must preserve this contract: bucket events by timestamp window; sum values per key per window; return deterministic window order. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Implement tumbling-window aggregation over event timestamps.
...
- bucket events by timestamp window
- sum values per key per window
- return deterministic window order
