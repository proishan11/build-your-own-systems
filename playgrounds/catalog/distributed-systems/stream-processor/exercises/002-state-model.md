# Exercise 002: Stream Processor State and Invariants

Shared concept chapter: [raft-and-replicated-logs.md](../../../../../curriculum/concepts/distributed-systems/raft-and-replicated-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Stream Processor**. The work is centered on `apply_stream_processor_stream_event`, not on a generic scaffold. You will implement behavior for **stream event** moving through **watermark state** while preserving the project invariant.

The implementation target is: Implement `apply_stream_processor_stream_event` so Stream Processor can replay stream events into watermark state without duplicating or accepting stale state.
...
watermark state must advance monotonically by version, and duplicate stream events must be idempotent.
...
1. What state does `apply_stream_processor_stream_event` own?
...
Implement `apply_stream_processor_stream_event` so Stream Processor can replay stream events into watermark state without duplicating or accepting stale state.
