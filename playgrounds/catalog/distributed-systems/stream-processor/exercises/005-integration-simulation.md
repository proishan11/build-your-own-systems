# Exercise 005: Stream Processor Integration Scenario

Shared concept chapter: [raft-and-replicated-logs.md](../../../../../curriculum/concepts/distributed-systems/raft-and-replicated-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Stream Processor**. The work is centered on `run_stream_processor_scenario`, not on a generic scaffold. You will implement behavior for **stream event** moving through **watermark state** while preserving the project invariant.

The implementation target is: Implement `run_stream_processor_scenario` so Stream Processor has a deterministic integration simulation with state, `windows_closed`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose watermark state state and windows_closed metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `watermark state=ready`, recording `windows_closed=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_stream_processor_scenario` own?
...
Implement `run_stream_processor_scenario` so Stream Processor has a deterministic integration simulation with state, `windows_closed`, failures, recoveries, and invariant reporting.
...
- track `windows_closed`, failures, and recoveries
