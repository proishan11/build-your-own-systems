# Exercise 004: Dynamo-Style KV Store Failure and Recovery

Shared concept chapter: [raft-and-replicated-logs.md](../../../../../curriculum/concepts/distributed-systems/raft-and-replicated-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Dynamo-Style KV Store**. The work is centered on `recover_dynamo_style_kv_store_sibling_conflict`, not on a generic scaffold. You will implement behavior for **kv write** moving through **replica set** while preserving the project invariant.

The implementation target is: Implement `recover_dynamo_style_kv_store_sibling_conflict` so Dynamo-Style KV Store handles transient failures, permanent `sibling conflict` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for replica set must never turn a permanent sibling conflict into an unsafe retry.
...
1. What state does `recover_dynamo_style_kv_store_sibling_conflict` own?
...
Implement `recover_dynamo_style_kv_store_sibling_conflict` so Dynamo-Style KV Store handles transient failures, permanent `sibling conflict` cases, and exhausted retry budgets explicitly.
...
- fail permanent `sibling conflict` reports immediately
