# Exercise 005: Replicated WAL With Raft Integration Scenario

Shared concept chapter: [raft-and-replicated-logs.md](../../../../../curriculum/concepts/distributed-systems/raft-and-replicated-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Replicated WAL With Raft**. The work is centered on `run_replicated_wal_with_raft_scenario`, not on a generic scaffold. You will implement behavior for **raft log entry** moving through **replica log** while preserving the project invariant.

The implementation target is: Implement `run_replicated_wal_with_raft_scenario` so Replicated WAL With Raft has a deterministic integration simulation with state, `commit_index`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose replica log state and commit_index metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `replica log=ready`, recording `commit_index=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_replicated_wal_with_raft_scenario` own?
...
Implement `run_replicated_wal_with_raft_scenario` so Replicated WAL With Raft has a deterministic integration simulation with state, `commit_index`, failures, recoveries, and invariant reporting.
...
- track `commit_index`, failures, and recoveries
