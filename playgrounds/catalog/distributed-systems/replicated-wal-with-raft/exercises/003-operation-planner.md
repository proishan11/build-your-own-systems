# Exercise 003: Replicated WAL With Raft Planning and Ordering

Shared concept chapter: [raft-and-replicated-logs.md](../../../../../curriculum/concepts/distributed-systems/raft-and-replicated-logs.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Replicated WAL With Raft**. The work is centered on `plan_replicated_wal_with_raft_append_entriess`, not on a generic scaffold. You will implement behavior for **raft log entry** moving through **replica log** while preserving the project invariant.

The implementation target is: Implement `plan_replicated_wal_with_raft_append_entriess` so Replicated WAL With Raft can produce a deterministic append entries plan from desired and observed replica log state.
...
The same desired and observed replica log states must always produce the same minimal ordered append entries plan.

## Tiny Example

If `replica-log-primary` is stale, `replica-log-old` is extra, and `replica-log-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_replicated_wal_with_raft_append_entriess` own?
...
Implement `plan_replicated_wal_with_raft_append_entriess` so Replicated WAL With Raft can produce a deterministic append entries plan from desired and observed replica log state.
