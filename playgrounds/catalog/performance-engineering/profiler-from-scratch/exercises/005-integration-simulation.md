# Exercise 005: Profiler From Scratch Integration Scenario

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Profiler From Scratch**. The work is centered on `run_profiler_from_scratch_scenario`, not on a generic scaffold. You will implement behavior for **stack sample** moving through **profile tree** while preserving the project invariant.

The implementation target is: Implement `run_profiler_from_scratch_scenario` so Profiler From Scratch has a deterministic integration simulation with state, `samples_recorded`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose profile tree state and samples_recorded metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `profile tree=ready`, recording `samples_recorded=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_profiler_from_scratch_scenario` own?
...
Implement `run_profiler_from_scratch_scenario` so Profiler From Scratch has a deterministic integration simulation with state, `samples_recorded`, failures, recoveries, and invariant reporting.
...
- track `samples_recorded`, failures, and recoveries
