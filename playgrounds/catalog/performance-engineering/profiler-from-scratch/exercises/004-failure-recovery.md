# Exercise 004: Profiler From Scratch Failure and Recovery

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Profiler From Scratch**. The work is centered on `recover_profiler_from_scratch_missing_frame`, not on a generic scaffold. You will implement behavior for **stack sample** moving through **profile tree** while preserving the project invariant.

The implementation target is: Implement `recover_profiler_from_scratch_missing_frame` so Profiler From Scratch handles transient failures, permanent `missing frame` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for profile tree must never turn a permanent missing frame into an unsafe retry.
...
1. What state does `recover_profiler_from_scratch_missing_frame` own?
...
Implement `recover_profiler_from_scratch_missing_frame` so Profiler From Scratch handles transient failures, permanent `missing frame` cases, and exhausted retry budgets explicitly.
...
- fail permanent `missing frame` reports immediately
