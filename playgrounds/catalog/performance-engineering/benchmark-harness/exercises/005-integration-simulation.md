# Exercise 005: Benchmark Harness Integration Scenario

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Benchmark Harness**. The work is centered on `run_benchmark_harness_scenario`, not on a generic scaffold. You will implement behavior for **benchmark sample** moving through **run config** while preserving the project invariant.

The implementation target is: Implement `run_benchmark_harness_scenario` so Benchmark Harness has a deterministic integration simulation with state, `confidence_interval`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose run config state and confidence_interval metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `run config=ready`, recording `confidence_interval=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_benchmark_harness_scenario` own?
...
Implement `run_benchmark_harness_scenario` so Benchmark Harness has a deterministic integration simulation with state, `confidence_interval`, failures, recoveries, and invariant reporting.
...
- track `confidence_interval`, failures, and recoveries
