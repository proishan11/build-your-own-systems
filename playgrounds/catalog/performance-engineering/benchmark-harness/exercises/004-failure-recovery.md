# Exercise 004: Benchmark Harness Failure and Recovery

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Benchmark Harness**. The work is centered on `recover_benchmark_harness_warmup_bias`, not on a generic scaffold. You will implement behavior for **benchmark sample** moving through **run config** while preserving the project invariant.

The implementation target is: Implement `recover_benchmark_harness_warmup_bias` so Benchmark Harness handles transient failures, permanent `warmup bias` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for run config must never turn a permanent warmup bias into an unsafe retry.
...
1. What state does `recover_benchmark_harness_warmup_bias` own?
...
Implement `recover_benchmark_harness_warmup_bias` so Benchmark Harness handles transient failures, permanent `warmup bias` cases, and exhausted retry budgets explicitly.
...
- fail permanent `warmup bias` reports immediately
