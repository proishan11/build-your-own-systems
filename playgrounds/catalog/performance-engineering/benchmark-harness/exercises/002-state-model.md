# Exercise 002: Benchmark Harness State and Invariants

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Benchmark Harness**. The work is centered on `apply_benchmark_harness_benchmark_sample_event`, not on a generic scaffold. You will implement behavior for **benchmark sample** moving through **run config** while preserving the project invariant.

The implementation target is: Implement `apply_benchmark_harness_benchmark_sample_event` so Benchmark Harness can replay benchmark sample events into run config without duplicating or accepting stale state.
...
run config must advance monotonically by version, and duplicate benchmark sample events must be idempotent.
...
1. What state does `apply_benchmark_harness_benchmark_sample_event` own?
...
Implement `apply_benchmark_harness_benchmark_sample_event` so Benchmark Harness can replay benchmark sample events into run config without duplicating or accepting stale state.
