# Exercise 002: Tail Latency Lab State and Invariants

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Tail Latency Lab**. The work is centered on `apply_tail_latency_lab_latency_sample_event`, not on a generic scaffold. You will implement behavior for **latency sample** moving through **histogram bucket** while preserving the project invariant.

The implementation target is: Implement `apply_tail_latency_lab_latency_sample_event` so Tail Latency Lab can replay latency sample events into histogram bucket without duplicating or accepting stale state.
...
histogram bucket must advance monotonically by version, and duplicate latency sample events must be idempotent.
...
1. What state does `apply_tail_latency_lab_latency_sample_event` own?
...
Implement `apply_tail_latency_lab_latency_sample_event` so Tail Latency Lab can replay latency sample events into histogram bucket without duplicating or accepting stale state.
