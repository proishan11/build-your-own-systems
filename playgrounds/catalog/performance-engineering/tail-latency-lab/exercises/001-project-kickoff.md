# Exercise 001: Tail Latency Lab Core Mechanism

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Tail Latency Lab**. The work is centered on `compute_percentile_latency_sample`, not on a generic scaffold. You will implement behavior for **latency sample** moving through **histogram bucket** while preserving the project invariant.

The implementation target is: Implement `compute_percentile_latency_sample` so Tail Latency Lab has a concrete implementation boundary for latency sample requests before they touch histogram bucket.
...
Only valid latency sample requests may become compute percentile operations against histogram bucket; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `latency-sample-001`, kind `compute percentile`, and target `histogram bucket` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `compute_percentile_latency_sample` own?
...
Implement `compute_percentile_latency_sample` so Tail Latency Lab has a concrete implementation boundary for latency sample requests before they touch histogram bucket.
...
- build valid latency sample requests into a stable project operation
...
- Treat `coordinated omission` as the kind of bad input that must never reach mutation code.
