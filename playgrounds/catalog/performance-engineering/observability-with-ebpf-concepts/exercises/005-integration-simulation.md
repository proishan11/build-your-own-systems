# Exercise 005: Observability With eBPF Concepts Integration Scenario

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Observability With eBPF Concepts**. The work is centered on `run_observability_with_ebpf_concepts_scenario`, not on a generic scaffold. You will implement behavior for **kernel event** moving through **probe registry** while preserving the project invariant.

The implementation target is: Implement `run_observability_with_ebpf_concepts_scenario` so Observability With eBPF Concepts has a deterministic integration simulation with state, `events_captured`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose probe registry state and events_captured metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `probe registry=ready`, recording `events_captured=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_observability_with_ebpf_concepts_scenario` own?
...
Implement `run_observability_with_ebpf_concepts_scenario` so Observability With eBPF Concepts has a deterministic integration simulation with state, `events_captured`, failures, recoveries, and invariant reporting.
...
- track `events_captured`, failures, and recoveries
