# Exercise 003: Observability With eBPF Concepts Planning and Ordering

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Observability With eBPF Concepts**. The work is centered on `plan_observability_with_ebpf_concepts_attach_probes`, not on a generic scaffold. You will implement behavior for **kernel event** moving through **probe registry** while preserving the project invariant.

The implementation target is: Implement `plan_observability_with_ebpf_concepts_attach_probes` so Observability With eBPF Concepts can produce a deterministic attach probe plan from desired and observed probe registry state.
...
The same desired and observed probe registry states must always produce the same minimal ordered attach probe plan.

## Tiny Example

If `probe-registry-primary` is stale, `probe-registry-old` is extra, and `probe-registry-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_observability_with_ebpf_concepts_attach_probes` own?
...
Implement `plan_observability_with_ebpf_concepts_attach_probes` so Observability With eBPF Concepts can produce a deterministic attach probe plan from desired and observed probe registry state.
