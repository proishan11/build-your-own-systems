# Exercise 002: Observability With eBPF Concepts State and Invariants

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Observability With eBPF Concepts**. The work is centered on `apply_observability_with_ebpf_concepts_kernel_event`, not on a generic scaffold. You will implement behavior for **kernel event** moving through **probe registry** while preserving the project invariant.

The implementation target is: Implement `apply_observability_with_ebpf_concepts_kernel_event` so Observability With eBPF Concepts can replay kernel events into probe registry without duplicating or accepting stale state.
...
probe registry must advance monotonically by version, and duplicate kernel events must be idempotent.
...
1. What state does `apply_observability_with_ebpf_concepts_kernel_event` own?
...
Implement `apply_observability_with_ebpf_concepts_kernel_event` so Observability With eBPF Concepts can replay kernel events into probe registry without duplicating or accepting stale state.
