# Exercise 005: Lock Contention Lab Integration Scenario

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Lock Contention Lab**. The work is centered on `run_lock_contention_lab_scenario`, not on a generic scaffold. You will implement behavior for **lock event** moving through **wait graph** while preserving the project invariant.

The implementation target is: Implement `run_lock_contention_lab_scenario` so Lock Contention Lab has a deterministic integration simulation with state, `wait_time_ms`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose wait graph state and wait_time_ms metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `wait graph=ready`, recording `wait_time_ms=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_lock_contention_lab_scenario` own?
...
Implement `run_lock_contention_lab_scenario` so Lock Contention Lab has a deterministic integration simulation with state, `wait_time_ms`, failures, recoveries, and invariant reporting.
...
- track `wait_time_ms`, failures, and recoveries
