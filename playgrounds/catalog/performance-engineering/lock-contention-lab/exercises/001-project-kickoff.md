# Exercise 001: Lock Contention Lab Core Mechanism

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Lock Contention Lab**. The work is centered on `attribute_wait_lock_event`, not on a generic scaffold. You will implement behavior for **lock event** moving through **wait graph** while preserving the project invariant.

The implementation target is: Implement `attribute_wait_lock_event` so Lock Contention Lab has a concrete implementation boundary for lock event requests before they touch wait graph.
...
Only valid lock event requests may become attribute wait operations against wait graph; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `lock-event-001`, kind `attribute wait`, and target `wait graph` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `attribute_wait_lock_event` own?
...
Implement `attribute_wait_lock_event` so Lock Contention Lab has a concrete implementation boundary for lock event requests before they touch wait graph.
...
- build valid lock event requests into a stable project operation
...
- Treat `convoy effect` as the kind of bad input that must never reach mutation code.
