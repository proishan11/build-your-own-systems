# Exercise 002: Lock Contention Lab State and Invariants

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Lock Contention Lab**. The work is centered on `apply_lock_contention_lab_lock_event`, not on a generic scaffold. You will implement behavior for **lock event** moving through **wait graph** while preserving the project invariant.

The implementation target is: Implement `apply_lock_contention_lab_lock_event` so Lock Contention Lab can replay lock events into wait graph without duplicating or accepting stale state.
...
wait graph must advance monotonically by version, and duplicate lock events must be idempotent.
...
1. What state does `apply_lock_contention_lab_lock_event` own?
...
Implement `apply_lock_contention_lab_lock_event` so Lock Contention Lab can replay lock events into wait graph without duplicating or accepting stale state.
