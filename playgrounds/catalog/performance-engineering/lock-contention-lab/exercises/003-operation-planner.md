# Exercise 003: Lock Contention Lab Planning and Ordering

Shared concept chapter: [profiling-and-tail-latency.md](../../../../../curriculum/concepts/performance/profiling-and-tail-latency.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Lock Contention Lab**. The work is centered on `plan_lock_contention_lab_attribute_waits`, not on a generic scaffold. You will implement behavior for **lock event** moving through **wait graph** while preserving the project invariant.

The implementation target is: Implement `plan_lock_contention_lab_attribute_waits` so Lock Contention Lab can produce a deterministic attribute wait plan from desired and observed wait graph state.
...
The same desired and observed wait graph states must always produce the same minimal ordered attribute wait plan.

## Tiny Example

If `wait-graph-primary` is stale, `wait-graph-old` is extra, and `wait-graph-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_lock_contention_lab_attribute_waits` own?
...
Implement `plan_lock_contention_lab_attribute_waits` so Lock Contention Lab can produce a deterministic attribute wait plan from desired and observed wait graph state.
