# Exercise 003: Agent Security Lab Planning and Ordering

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Agent Security Lab**. The work is centered on `plan_agent_security_lab_classify_tools`, not on a generic scaffold. You will implement behavior for **agent tool call** moving through **tool policy** while preserving the project invariant.

The implementation target is: Implement `plan_agent_security_lab_classify_tools` so Agent Security Lab can produce a deterministic classify tool plan from desired and observed tool policy state.
...
The same desired and observed tool policy states must always produce the same minimal ordered classify tool plan.

## Tiny Example

If `tool-policy-primary` is stale, `tool-policy-old` is extra, and `tool-policy-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_agent_security_lab_classify_tools` own?
...
Implement `plan_agent_security_lab_classify_tools` so Agent Security Lab can produce a deterministic classify tool plan from desired and observed tool policy state.
