# Exercise 005: Agent Security Lab Integration Scenario

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Agent Security Lab**. The work is centered on `run_agent_security_lab_scenario`, not on a generic scaffold. You will implement behavior for **agent tool call** moving through **tool policy** while preserving the project invariant.

The implementation target is: Implement `run_agent_security_lab_scenario` so Agent Security Lab has a deterministic integration simulation with state, `calls_denied`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose tool policy state and calls_denied metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `tool policy=ready`, recording `calls_denied=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_agent_security_lab_scenario` own?
...
Implement `run_agent_security_lab_scenario` so Agent Security Lab has a deterministic integration simulation with state, `calls_denied`, failures, recoveries, and invariant reporting.
...
- track `calls_denied`, failures, and recoveries
