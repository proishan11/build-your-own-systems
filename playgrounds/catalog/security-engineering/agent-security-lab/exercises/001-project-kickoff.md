# Exercise 001: Agent Security Lab Core Mechanism

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Agent Security Lab**. The work is centered on `classify_tool_agent_tool_call`, not on a generic scaffold. You will implement behavior for **agent tool call** moving through **tool policy** while preserving the project invariant.

The implementation target is: Implement `classify_tool_agent_tool_call` so Agent Security Lab has a concrete implementation boundary for agent tool call requests before they touch tool policy.
...
Only valid agent tool call requests may become classify tool operations against tool policy; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `agent-tool-call-001`, kind `classify tool`, and target `tool policy` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `classify_tool_agent_tool_call` own?
...
Implement `classify_tool_agent_tool_call` so Agent Security Lab has a concrete implementation boundary for agent tool call requests before they touch tool policy.
...
- build valid agent tool call requests into a stable project operation
...
- Treat `secret exfiltration` as the kind of bad input that must never reach mutation code.
