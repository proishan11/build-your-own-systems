# Exercise 004: Agent Security Lab Failure and Recovery

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Agent Security Lab**. The work is centered on `recover_agent_security_lab_secret_exfiltration`, not on a generic scaffold. You will implement behavior for **agent tool call** moving through **tool policy** while preserving the project invariant.

The implementation target is: Implement `recover_agent_security_lab_secret_exfiltration` so Agent Security Lab handles transient failures, permanent `secret exfiltration` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for tool policy must never turn a permanent secret exfiltration into an unsafe retry.
...
1. What state does `recover_agent_security_lab_secret_exfiltration` own?
...
Implement `recover_agent_security_lab_secret_exfiltration` so Agent Security Lab handles transient failures, permanent `secret exfiltration` cases, and exhausted retry budgets explicitly.
...
- fail permanent `secret exfiltration` reports immediately
