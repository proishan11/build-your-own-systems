# Exercise 002: Vulnerable Web App Lab State and Invariants

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Vulnerable Web App Lab**. The work is centered on `apply_vulnerable_web_app_lab_http_input_event`, not on a generic scaffold. You will implement behavior for **http input** moving through **security policy** while preserving the project invariant.

The implementation target is: Implement `apply_vulnerable_web_app_lab_http_input_event` so Vulnerable Web App Lab can replay http input events into security policy without duplicating or accepting stale state.
...
security policy must advance monotonically by version, and duplicate http input events must be idempotent.
...
1. What state does `apply_vulnerable_web_app_lab_http_input_event` own?
...
Implement `apply_vulnerable_web_app_lab_http_input_event` so Vulnerable Web App Lab can replay http input events into security policy without duplicating or accepting stale state.
