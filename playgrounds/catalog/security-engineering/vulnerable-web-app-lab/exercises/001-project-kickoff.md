# Exercise 001: Vulnerable Web App Lab Core Mechanism

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Vulnerable Web App Lab**. The work is centered on `sanitize_request_http_input`, not on a generic scaffold. You will implement behavior for **http input** moving through **security policy** while preserving the project invariant.

The implementation target is: Implement `sanitize_request_http_input` so Vulnerable Web App Lab has a concrete implementation boundary for http input requests before they touch security policy.
...
Only valid http input requests may become sanitize request operations against security policy; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `http-input-001`, kind `sanitize request`, and target `security policy` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `sanitize_request_http_input` own?
...
Implement `sanitize_request_http_input` so Vulnerable Web App Lab has a concrete implementation boundary for http input requests before they touch security policy.
...
- build valid http input requests into a stable project operation
...
- Treat `xss payload` as the kind of bad input that must never reach mutation code.
