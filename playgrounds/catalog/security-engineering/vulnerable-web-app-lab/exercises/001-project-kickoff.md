# Exercise 001: Vulnerable Web App Lab First Implementation Lab

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This first lab turns **Vulnerable Web App Lab** from a broad project idea into a concrete implementation problem. You are building one narrow mechanism, but you should treat it like production code: name the invariant, make the state explicit, and let the tests describe externally visible behavior.

The implementation target is: Fix broken object-level authorization in a tiny handler.
...
After each public operation, the component must preserve this contract: allow owners to access their records; allow admins to access all records; deny other users. If a later feature makes this invariant harder to maintain, the design should expose that tension instead of hiding it in incidental code.
...
Fix broken object-level authorization in a tiny handler.
...
- allow owners to access their records
