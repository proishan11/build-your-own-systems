# Exercise 003: Vulnerable Web App Lab Planning and Ordering

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Vulnerable Web App Lab**. The work is centered on `plan_vulnerable_web_app_lab_sanitize_requests`, not on a generic scaffold. You will implement behavior for **http input** moving through **security policy** while preserving the project invariant.

The implementation target is: Implement `plan_vulnerable_web_app_lab_sanitize_requests` so Vulnerable Web App Lab can produce a deterministic sanitize request plan from desired and observed security policy state.
...
The same desired and observed security policy states must always produce the same minimal ordered sanitize request plan.

## Tiny Example

If `security-policy-primary` is stale, `security-policy-old` is extra, and `security-policy-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_vulnerable_web_app_lab_sanitize_requests` own?
...
Implement `plan_vulnerable_web_app_lab_sanitize_requests` so Vulnerable Web App Lab can produce a deterministic sanitize request plan from desired and observed security policy state.
