# Exercise 002: Auth and Session System State and Invariants

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Auth and Session System**. The work is centered on `apply_auth_and_session_system_session_token_event`, not on a generic scaffold. You will implement behavior for **session token** moving through **session store** while preserving the project invariant.

The implementation target is: Implement `apply_auth_and_session_system_session_token_event` so Auth and Session System can replay session token events into session store without duplicating or accepting stale state.
...
session store must advance monotonically by version, and duplicate session token events must be idempotent.
...
1. What state does `apply_auth_and_session_system_session_token_event` own?
...
Implement `apply_auth_and_session_system_session_token_event` so Auth and Session System can replay session token events into session store without duplicating or accepting stale state.
