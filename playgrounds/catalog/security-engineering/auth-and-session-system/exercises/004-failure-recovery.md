# Exercise 004: Auth and Session System Failure and Recovery

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Auth and Session System**. The work is centered on `recover_auth_and_session_system_session_fixation`, not on a generic scaffold. You will implement behavior for **session token** moving through **session store** while preserving the project invariant.

The implementation target is: Implement `recover_auth_and_session_system_session_fixation` so Auth and Session System handles transient failures, permanent `session fixation` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for session store must never turn a permanent session fixation into an unsafe retry.
...
1. What state does `recover_auth_and_session_system_session_fixation` own?
...
Implement `recover_auth_and_session_system_session_fixation` so Auth and Session System handles transient failures, permanent `session fixation` cases, and exhausted retry budgets explicitly.
...
- fail permanent `session fixation` reports immediately
