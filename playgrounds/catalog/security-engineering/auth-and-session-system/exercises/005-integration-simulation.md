# Exercise 005: Auth and Session System Integration Scenario

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Auth and Session System**. The work is centered on `run_auth_and_session_system_scenario`, not on a generic scaffold. You will implement behavior for **session token** moving through **session store** while preserving the project invariant.

The implementation target is: Implement `run_auth_and_session_system_scenario` so Auth and Session System has a deterministic integration simulation with state, `sessions_revoked`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose session store state and sessions_revoked metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `session store=ready`, recording `sessions_revoked=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_auth_and_session_system_scenario` own?
...
Implement `run_auth_and_session_system_scenario` so Auth and Session System has a deterministic integration simulation with state, `sessions_revoked`, failures, recoveries, and invariant reporting.
...
- track `sessions_revoked`, failures, and recoveries
