# Exercise 005: Secrets Manager Integration Scenario

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Secrets Manager**. The work is centered on `run_secrets_manager_scenario`, not on a generic scaffold. You will implement behavior for **secret version** moving through **secret store** while preserving the project invariant.

The implementation target is: Implement `run_secrets_manager_scenario` so Secrets Manager has a deterministic integration simulation with state, `rotations_completed`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose secret store state and rotations_completed metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `secret store=ready`, recording `rotations_completed=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_secrets_manager_scenario` own?
...
Implement `run_secrets_manager_scenario` so Secrets Manager has a deterministic integration simulation with state, `rotations_completed`, failures, recoveries, and invariant reporting.
...
- track `rotations_completed`, failures, and recoveries
