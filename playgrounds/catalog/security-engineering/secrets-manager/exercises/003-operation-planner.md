# Exercise 003: Secrets Manager Planning and Ordering

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Secrets Manager**. The work is centered on `plan_secrets_manager_rotate_secrets`, not on a generic scaffold. You will implement behavior for **secret version** moving through **secret store** while preserving the project invariant.

The implementation target is: Implement `plan_secrets_manager_rotate_secrets` so Secrets Manager can produce a deterministic rotate secret plan from desired and observed secret store state.
...
The same desired and observed secret store states must always produce the same minimal ordered rotate secret plan.

## Tiny Example

If `secret-store-primary` is stale, `secret-store-old` is extra, and `secret-store-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_secrets_manager_rotate_secrets` own?
...
Implement `plan_secrets_manager_rotate_secrets` so Secrets Manager can produce a deterministic rotate secret plan from desired and observed secret store state.
