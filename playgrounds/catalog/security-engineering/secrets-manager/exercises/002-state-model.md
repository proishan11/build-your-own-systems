# Exercise 002: Secrets Manager State and Invariants

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Secrets Manager**. The work is centered on `apply_secrets_manager_secret_version_event`, not on a generic scaffold. You will implement behavior for **secret version** moving through **secret store** while preserving the project invariant.

The implementation target is: Implement `apply_secrets_manager_secret_version_event` so Secrets Manager can replay secret version events into secret store without duplicating or accepting stale state.
...
secret store must advance monotonically by version, and duplicate secret version events must be idempotent.
...
1. What state does `apply_secrets_manager_secret_version_event` own?
...
Implement `apply_secrets_manager_secret_version_event` so Secrets Manager can replay secret version events into secret store without duplicating or accepting stale state.
