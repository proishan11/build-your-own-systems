# Exercise 004: Supply-Chain Scanner Failure and Recovery

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Supply-Chain Scanner**. The work is centered on `recover_supply_chain_scanner_unsigned_dependency`, not on a generic scaffold. You will implement behavior for **package artifact** moving through **sbom index** while preserving the project invariant.

The implementation target is: Implement `recover_supply_chain_scanner_unsigned_dependency` so Supply-Chain Scanner handles transient failures, permanent `unsigned dependency` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for sbom index must never turn a permanent unsigned dependency into an unsafe retry.
...
1. What state does `recover_supply_chain_scanner_unsigned_dependency` own?
...
Implement `recover_supply_chain_scanner_unsigned_dependency` so Supply-Chain Scanner handles transient failures, permanent `unsigned dependency` cases, and exhausted retry budgets explicitly.
...
- fail permanent `unsigned dependency` reports immediately
