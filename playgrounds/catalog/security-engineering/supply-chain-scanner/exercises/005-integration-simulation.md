# Exercise 005: Supply-Chain Scanner Integration Scenario

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Supply-Chain Scanner**. The work is centered on `run_supply_chain_scanner_scenario`, not on a generic scaffold. You will implement behavior for **package artifact** moving through **sbom index** while preserving the project invariant.

The implementation target is: Implement `run_supply_chain_scanner_scenario` so Supply-Chain Scanner has a deterministic integration simulation with state, `findings_reported`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose sbom index state and findings_reported metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `sbom index=ready`, recording `findings_reported=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_supply_chain_scanner_scenario` own?
...
Implement `run_supply_chain_scanner_scenario` so Supply-Chain Scanner has a deterministic integration simulation with state, `findings_reported`, failures, recoveries, and invariant reporting.
...
- track `findings_reported`, failures, and recoveries
