# Exercise 003: Supply-Chain Scanner Planning and Ordering

Shared concept chapter: [threat-modeling.md](../../../../../curriculum/concepts/security/threat-modeling.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Supply-Chain Scanner**. The work is centered on `plan_supply_chain_scanner_scan_artifacts`, not on a generic scaffold. You will implement behavior for **package artifact** moving through **sbom index** while preserving the project invariant.

The implementation target is: Implement `plan_supply_chain_scanner_scan_artifacts` so Supply-Chain Scanner can produce a deterministic scan artifact plan from desired and observed sbom index state.
...
The same desired and observed sbom index states must always produce the same minimal ordered scan artifact plan.

## Tiny Example

If `sbom-index-primary` is stale, `sbom-index-old` is extra, and `sbom-index-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_supply_chain_scanner_scan_artifacts` own?
...
Implement `plan_supply_chain_scanner_scan_artifacts` so Supply-Chain Scanner can produce a deterministic scan artifact plan from desired and observed sbom index state.
