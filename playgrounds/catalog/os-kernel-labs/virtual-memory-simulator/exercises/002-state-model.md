# Exercise 002: Virtual Memory Simulator State and Invariants
...
This exercise is a project-specific implementation milestone for **Virtual Memory Simulator**. The work is centered on `apply_virtual_memory_simulator_virtual_page_event`, not on a generic scaffold. You will implement behavior for **virtual page** moving through **page table** while preserving the project invariant.

The implementation target is: Implement `apply_virtual_memory_simulator_virtual_page_event` so Virtual Memory Simulator can replay virtual page events into page table without duplicating or accepting stale state.
...
page table must advance monotonically by version, and duplicate virtual page events must be idempotent.
...
1. What state does `apply_virtual_memory_simulator_virtual_page_event` own?
...
Implement `apply_virtual_memory_simulator_virtual_page_event` so Virtual Memory Simulator can replay virtual page events into page table without duplicating or accepting stale state.
