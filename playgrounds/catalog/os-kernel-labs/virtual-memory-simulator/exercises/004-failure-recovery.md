# Exercise 004: Virtual Memory Simulator Failure and Recovery
...
This exercise is a project-specific implementation milestone for **Virtual Memory Simulator**. The work is centered on `recover_virtual_memory_simulator_permission_fault`, not on a generic scaffold. You will implement behavior for **virtual page** moving through **page table** while preserving the project invariant.

The implementation target is: Implement `recover_virtual_memory_simulator_permission_fault` so Virtual Memory Simulator handles transient failures, permanent `permission fault` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for page table must never turn a permanent permission fault into an unsafe retry.
...
1. What state does `recover_virtual_memory_simulator_permission_fault` own?
...
Implement `recover_virtual_memory_simulator_permission_fault` so Virtual Memory Simulator handles transient failures, permanent `permission fault` cases, and exhausted retry budgets explicitly.
...
- fail permanent `permission fault` reports immediately
