# Exercise 002: Tiny Journaling File System State and Invariants
...
This exercise is a project-specific implementation milestone for **Tiny Journaling File System**. The work is centered on `apply_tiny_journaling_file_system_journal_transaction_event`, not on a generic scaffold. You will implement behavior for **journal transaction** moving through **inode block** while preserving the project invariant.

The implementation target is: Implement `apply_tiny_journaling_file_system_journal_transaction_event` so Tiny Journaling File System can replay journal transaction events into inode block without duplicating or accepting stale state.
...
inode block must advance monotonically by version, and duplicate journal transaction events must be idempotent.
...
1. What state does `apply_tiny_journaling_file_system_journal_transaction_event` own?
...
Implement `apply_tiny_journaling_file_system_journal_transaction_event` so Tiny Journaling File System can replay journal transaction events into inode block without duplicating or accepting stale state.
