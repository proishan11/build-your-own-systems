# Exercise 004: Tiny Journaling File System Failure and Recovery
...
This exercise is a project-specific implementation milestone for **Tiny Journaling File System**. The work is centered on `recover_tiny_journaling_file_system_torn_journal_write`, not on a generic scaffold. You will implement behavior for **journal transaction** moving through **inode block** while preserving the project invariant.

The implementation target is: Implement `recover_tiny_journaling_file_system_torn_journal_write` so Tiny Journaling File System handles transient failures, permanent `torn journal write` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for inode block must never turn a permanent torn journal write into an unsafe retry.
...
1. What state does `recover_tiny_journaling_file_system_torn_journal_write` own?
...
Implement `recover_tiny_journaling_file_system_torn_journal_write` so Tiny Journaling File System handles transient failures, permanent `torn journal write` cases, and exhausted retry budgets explicitly.
...
- fail permanent `torn journal write` reports immediately
