# Exercise 005: Tiny Journaling File System Integration Scenario
...
This exercise is a project-specific implementation milestone for **Tiny Journaling File System**. The work is centered on `run_tiny_journaling_file_system_scenario`, not on a generic scaffold. You will implement behavior for **journal transaction** moving through **inode block** while preserving the project invariant.

The implementation target is: Implement `run_tiny_journaling_file_system_scenario` so Tiny Journaling File System has a deterministic integration simulation with state, `replayed_transactions`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose inode block state and replayed_transactions metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `inode block=ready`, recording `replayed_transactions=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_tiny_journaling_file_system_scenario` own?
...
Implement `run_tiny_journaling_file_system_scenario` so Tiny Journaling File System has a deterministic integration simulation with state, `replayed_transactions`, failures, recoveries, and invariant reporting.
...
- track `replayed_transactions`, failures, and recoveries
