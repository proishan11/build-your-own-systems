# Exercise 003: Tiny Journaling File System Planning and Ordering
...
This exercise is a project-specific implementation milestone for **Tiny Journaling File System**. The work is centered on `plan_tiny_journaling_file_system_commit_records`, not on a generic scaffold. You will implement behavior for **journal transaction** moving through **inode block** while preserving the project invariant.

The implementation target is: Implement `plan_tiny_journaling_file_system_commit_records` so Tiny Journaling File System can produce a deterministic commit record plan from desired and observed inode block state.
...
The same desired and observed inode block states must always produce the same minimal ordered commit record plan.

## Tiny Example

If `inode-block-primary` is stale, `inode-block-old` is extra, and `inode-block-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_tiny_journaling_file_system_commit_records` own?
...
Implement `plan_tiny_journaling_file_system_commit_records` so Tiny Journaling File System can produce a deterministic commit record plan from desired and observed inode block state.
