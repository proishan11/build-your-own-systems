# Exercise 003: xv6-Style Kernel Lab Planning and Ordering
...
This exercise is a project-specific implementation milestone for **xv6-Style Kernel Lab**. The work is centered on `plan_xv6_style_kernel_lab_enter_kernels`, not on a generic scaffold. You will implement behavior for **system call** moving through **process table** while preserving the project invariant.

The implementation target is: Implement `plan_xv6_style_kernel_lab_enter_kernels` so xv6-Style Kernel Lab can produce a deterministic enter kernel plan from desired and observed process table state.
...
The same desired and observed process table states must always produce the same minimal ordered enter kernel plan.

## Tiny Example

If `process-table-primary` is stale, `process-table-old` is extra, and `process-table-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_xv6_style_kernel_lab_enter_kernels` own?
...
Implement `plan_xv6_style_kernel_lab_enter_kernels` so xv6-Style Kernel Lab can produce a deterministic enter kernel plan from desired and observed process table state.
