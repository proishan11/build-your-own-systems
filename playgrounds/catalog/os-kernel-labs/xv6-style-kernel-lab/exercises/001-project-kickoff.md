# Exercise 001: xv6-Style Kernel Lab Core Mechanism
...
This exercise is a project-specific implementation milestone for **xv6-Style Kernel Lab**. The work is centered on `enter_kernel_system_call`, not on a generic scaffold. You will implement behavior for **system call** moving through **process table** while preserving the project invariant.

The implementation target is: Implement `enter_kernel_system_call` so xv6-Style Kernel Lab has a concrete implementation boundary for system call requests before they touch process table.
...
Only valid system call requests may become enter kernel operations against process table; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `system-call-001`, kind `enter kernel`, and target `process table` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `enter_kernel_system_call` own?
...
Implement `enter_kernel_system_call` so xv6-Style Kernel Lab has a concrete implementation boundary for system call requests before they touch process table.
...
- build valid system call requests into a stable project operation
...
- Treat `invalid trap frame` as the kind of bad input that must never reach mutation code.
