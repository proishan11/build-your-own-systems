# Exercise 005: User-Level Thread Library Integration Scenario
...
This exercise is a project-specific implementation milestone for **User-Level Thread Library**. The work is centered on `run_user_level_thread_library_scenario`, not on a generic scaffold. You will implement behavior for **green thread** moving through **run queue** while preserving the project invariant.

The implementation target is: Implement `run_user_level_thread_library_scenario` so User-Level Thread Library has a deterministic integration simulation with state, `context_switches`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose run queue state and context_switches metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `run queue=ready`, recording `context_switches=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_user_level_thread_library_scenario` own?
...
Implement `run_user_level_thread_library_scenario` so User-Level Thread Library has a deterministic integration simulation with state, `context_switches`, failures, recoveries, and invariant reporting.
...
- track `context_switches`, failures, and recoveries
