# Exercise 004: User-Level Thread Library Failure and Recovery
...
This exercise is a project-specific implementation milestone for **User-Level Thread Library**. The work is centered on `recover_user_level_thread_library_lost_wakeup`, not on a generic scaffold. You will implement behavior for **green thread** moving through **run queue** while preserving the project invariant.

The implementation target is: Implement `recover_user_level_thread_library_lost_wakeup` so User-Level Thread Library handles transient failures, permanent `lost wakeup` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for run queue must never turn a permanent lost wakeup into an unsafe retry.
...
1. What state does `recover_user_level_thread_library_lost_wakeup` own?
...
Implement `recover_user_level_thread_library_lost_wakeup` so User-Level Thread Library handles transient failures, permanent `lost wakeup` cases, and exhausted retry budgets explicitly.
...
- fail permanent `lost wakeup` reports immediately
