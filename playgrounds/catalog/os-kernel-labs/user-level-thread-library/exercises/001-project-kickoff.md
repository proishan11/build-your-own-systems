# Exercise 001: User-Level Thread Library Core Mechanism
...
This exercise is a project-specific implementation milestone for **User-Level Thread Library**. The work is centered on `yield_thread_green_thread`, not on a generic scaffold. You will implement behavior for **green thread** moving through **run queue** while preserving the project invariant.

The implementation target is: Implement `yield_thread_green_thread` so User-Level Thread Library has a concrete implementation boundary for green thread requests before they touch run queue.
...
Only valid green thread requests may become yield thread operations against run queue; malformed input must produce a deterministic rejection.

## Tiny Example

A valid request with id `green-thread-001`, kind `yield thread`, and target `run queue` becomes a concrete operation. A request with an empty kind is rejected before it can mutate state.
...
1. What state does `yield_thread_green_thread` own?
...
Implement `yield_thread_green_thread` so User-Level Thread Library has a concrete implementation boundary for green thread requests before they touch run queue.
...
- build valid green thread requests into a stable project operation
...
- Treat `lost wakeup` as the kind of bad input that must never reach mutation code.
