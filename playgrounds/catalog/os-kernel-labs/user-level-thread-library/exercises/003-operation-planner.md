# Exercise 003: User-Level Thread Library Planning and Ordering
...
This exercise is a project-specific implementation milestone for **User-Level Thread Library**. The work is centered on `plan_user_level_thread_library_yield_threads`, not on a generic scaffold. You will implement behavior for **green thread** moving through **run queue** while preserving the project invariant.

The implementation target is: Implement `plan_user_level_thread_library_yield_threads` so User-Level Thread Library can produce a deterministic yield thread plan from desired and observed run queue state.
...
The same desired and observed run queue states must always produce the same minimal ordered yield thread plan.

## Tiny Example

If `run-queue-primary` is stale, `run-queue-old` is extra, and `run-queue-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_user_level_thread_library_yield_threads` own?
...
Implement `plan_user_level_thread_library_yield_threads` so User-Level Thread Library can produce a deterministic yield thread plan from desired and observed run queue state.
