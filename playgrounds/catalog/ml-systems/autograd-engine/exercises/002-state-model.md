# Exercise 002: Autograd Engine State and Invariants

Shared concept chapter: [autograd-and-training-loops.md](../../../../../curriculum/concepts/ml-systems/autograd-and-training-loops.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Autograd Engine**. The work is centered on `apply_autograd_engine_tensor_operation_event`, not on a generic scaffold. You will implement behavior for **tensor operation** moving through **computation graph** while preserving the project invariant.

The implementation target is: Implement `apply_autograd_engine_tensor_operation_event` so Autograd Engine can replay tensor operation events into computation graph without duplicating or accepting stale state.
...
computation graph must advance monotonically by version, and duplicate tensor operation events must be idempotent.
...
1. What state does `apply_autograd_engine_tensor_operation_event` own?
...
Implement `apply_autograd_engine_tensor_operation_event` so Autograd Engine can replay tensor operation events into computation graph without duplicating or accepting stale state.
