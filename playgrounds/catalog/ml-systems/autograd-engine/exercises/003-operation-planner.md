# Exercise 003: Autograd Engine Planning and Ordering

Shared concept chapter: [autograd-and-training-loops.md](../../../../../curriculum/concepts/ml-systems/autograd-and-training-loops.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Autograd Engine**. The work is centered on `plan_autograd_engine_backpropagate_gradients`, not on a generic scaffold. You will implement behavior for **tensor operation** moving through **computation graph** while preserving the project invariant.

The implementation target is: Implement `plan_autograd_engine_backpropagate_gradients` so Autograd Engine can produce a deterministic backpropagate gradient plan from desired and observed computation graph state.
...
The same desired and observed computation graph states must always produce the same minimal ordered backpropagate gradient plan.

## Tiny Example

If `computation-graph-primary` is stale, `computation-graph-old` is extra, and `computation-graph-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_autograd_engine_backpropagate_gradients` own?
...
Implement `plan_autograd_engine_backpropagate_gradients` so Autograd Engine can produce a deterministic backpropagate gradient plan from desired and observed computation graph state.
