# Exercise 004: Mini Deep Learning Framework Failure and Recovery

Shared concept chapter: [autograd-and-training-loops.md](../../../../../curriculum/concepts/ml-systems/autograd-and-training-loops.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Mini Deep Learning Framework**. The work is centered on `recover_mini_deep_learning_framework_nan_gradient`, not on a generic scaffold. You will implement behavior for **training batch** moving through **module graph** while preserving the project invariant.

The implementation target is: Implement `recover_mini_deep_learning_framework_nan_gradient` so Mini Deep Learning Framework handles transient failures, permanent `nan gradient` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for module graph must never turn a permanent nan gradient into an unsafe retry.
...
1. What state does `recover_mini_deep_learning_framework_nan_gradient` own?
...
Implement `recover_mini_deep_learning_framework_nan_gradient` so Mini Deep Learning Framework handles transient failures, permanent `nan gradient` cases, and exhausted retry budgets explicitly.
...
- fail permanent `nan gradient` reports immediately
