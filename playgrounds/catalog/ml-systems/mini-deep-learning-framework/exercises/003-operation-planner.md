# Exercise 003: Mini Deep Learning Framework Planning and Ordering

Shared concept chapter: [autograd-and-training-loops.md](../../../../../curriculum/concepts/ml-systems/autograd-and-training-loops.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Mini Deep Learning Framework**. The work is centered on `plan_mini_deep_learning_framework_run_forward_passs`, not on a generic scaffold. You will implement behavior for **training batch** moving through **module graph** while preserving the project invariant.

The implementation target is: Implement `plan_mini_deep_learning_framework_run_forward_passs` so Mini Deep Learning Framework can produce a deterministic run forward pass plan from desired and observed module graph state.
...
The same desired and observed module graph states must always produce the same minimal ordered run forward pass plan.

## Tiny Example

If `module-graph-primary` is stale, `module-graph-old` is extra, and `module-graph-canary` is missing, the plan updates, deletes, then creates in stable order.
...
1. What state does `plan_mini_deep_learning_framework_run_forward_passs` own?
...
Implement `plan_mini_deep_learning_framework_run_forward_passs` so Mini Deep Learning Framework can produce a deterministic run forward pass plan from desired and observed module graph state.
