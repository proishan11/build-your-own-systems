# Exercise 005: Feature Store Integration Scenario

Shared concept chapter: [autograd-and-training-loops.md](../../../../../curriculum/concepts/ml-systems/autograd-and-training-loops.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Feature Store**. The work is centered on `run_feature_store_scenario`, not on a generic scaffold. You will implement behavior for **feature row** moving through **feature registry** while preserving the project invariant.

The implementation target is: Implement `run_feature_store_scenario` so Feature Store has a deterministic integration simulation with state, `fresh_rows`, failures, recoveries, and invariant reporting.
...
The scenario runner must expose feature registry state and fresh_rows metrics while reporting malformed events as violations instead of hiding them.

## Tiny Example

Applying `feature registry=ready`, recording `fresh_rows=3`, failing, and recovering should preserve state and produce one failure plus one recovery.
...
1. What state does `run_feature_store_scenario` own?
...
Implement `run_feature_store_scenario` so Feature Store has a deterministic integration simulation with state, `fresh_rows`, failures, recoveries, and invariant reporting.
...
- track `fresh_rows`, failures, and recoveries
