# Exercise 004: Distributed Training Simulator Failure and Recovery

Shared concept chapter: [autograd-and-training-loops.md](../../../../../curriculum/concepts/ml-systems/autograd-and-training-loops.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Distributed Training Simulator**. The work is centered on `recover_distributed_training_simulator_straggler_worker`, not on a generic scaffold. You will implement behavior for **gradient shard** moving through **worker ring** while preserving the project invariant.

The implementation target is: Implement `recover_distributed_training_simulator_straggler_worker` so Distributed Training Simulator handles transient failures, permanent `straggler worker` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for worker ring must never turn a permanent straggler worker into an unsafe retry.
...
1. What state does `recover_distributed_training_simulator_straggler_worker` own?
...
Implement `recover_distributed_training_simulator_straggler_worker` so Distributed Training Simulator handles transient failures, permanent `straggler worker` cases, and exhausted retry budgets explicitly.
...
- fail permanent `straggler worker` reports immediately
