# Exercise 004: Go Concurrency Gauntlet Failure and Recovery

Shared concept chapter: [pipelines-and-cancellation.md](../../../../../curriculum/concepts/go-concurrency/pipelines-and-cancellation.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Go Concurrency Gauntlet**. The work is centered on `recover_go_concurrency_gauntlet_blocked_sender`, not on a generic scaffold. You will implement behavior for **concurrent job** moving through **worker group** while preserving the project invariant.

The implementation target is: Implement `recover_go_concurrency_gauntlet_blocked_sender` so Go Concurrency Gauntlet handles transient failures, permanent `blocked sender` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for worker group must never turn a permanent blocked sender into an unsafe retry.
...
1. What state does `recover_go_concurrency_gauntlet_blocked_sender` own?
...
Implement `recover_go_concurrency_gauntlet_blocked_sender` so Go Concurrency Gauntlet handles transient failures, permanent `blocked sender` cases, and exhausted retry budgets explicitly.
...
- fail permanent `blocked sender` reports immediately
