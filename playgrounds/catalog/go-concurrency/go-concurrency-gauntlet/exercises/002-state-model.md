# Exercise 002: Go Concurrency Gauntlet State and Invariants

Shared concept chapter: [pipelines-and-cancellation.md](../../../../../curriculum/concepts/go-concurrency/pipelines-and-cancellation.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **Go Concurrency Gauntlet**. The work is centered on `apply_go_concurrency_gauntlet_concurrent_job_event`, not on a generic scaffold. You will implement behavior for **concurrent job** moving through **worker group** while preserving the project invariant.

The implementation target is: Implement `apply_go_concurrency_gauntlet_concurrent_job_event` so Go Concurrency Gauntlet can replay concurrent job events into worker group without duplicating or accepting stale state.
...
worker group must advance monotonically by version, and duplicate concurrent job events must be idempotent.
...
1. What state does `apply_go_concurrency_gauntlet_concurrent_job_event` own?
...
Implement `apply_go_concurrency_gauntlet_concurrent_job_event` so Go Concurrency Gauntlet can replay concurrent job events into worker group without duplicating or accepting stale state.
