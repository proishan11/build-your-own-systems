# Exercise 002: DNS Resolver State and Invariants

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **DNS Resolver**. The work is centered on `apply_dns_resolver_dns_question_event`, not on a generic scaffold. You will implement behavior for **dns question** moving through **resolver cache** while preserving the project invariant.

The implementation target is: Implement `apply_dns_resolver_dns_question_event` so DNS Resolver can replay dns question events into resolver cache without duplicating or accepting stale state.
...
resolver cache must advance monotonically by version, and duplicate dns question events must be idempotent.
...
1. What state does `apply_dns_resolver_dns_question_event` own?
...
Implement `apply_dns_resolver_dns_question_event` so DNS Resolver can replay dns question events into resolver cache without duplicating or accepting stale state.
