# Exercise 004: DNS Resolver Failure and Recovery

Shared concept chapter: [reliable-transport.md](../../../../../curriculum/concepts/networking/reliable-transport.md)

## Concept Primer

This exercise is a project-specific implementation milestone for **DNS Resolver**. The work is centered on `recover_dns_resolver_ttl_expiry`, not on a generic scaffold. You will implement behavior for **dns question** moving through **resolver cache** while preserving the project invariant.

The implementation target is: Implement `recover_dns_resolver_ttl_expiry` so DNS Resolver handles transient failures, permanent `ttl expiry` cases, and exhausted retry budgets explicitly.
...
Recovery decisions for resolver cache must never turn a permanent ttl expiry into an unsafe retry.
...
1. What state does `recover_dns_resolver_ttl_expiry` own?
...
Implement `recover_dns_resolver_ttl_expiry` so DNS Resolver handles transient failures, permanent `ttl expiry` cases, and exhausted retry budgets explicitly.
...
- fail permanent `ttl expiry` reports immediately
